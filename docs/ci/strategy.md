...
- Merge-gate policy command:
  - `bash scripts/ci/check_anti_flake_policy.sh --registry-file .ci/flaky-tests.txt --expected-final-decision GO --max-active-entries 0 --fast-workflow-file .github/workflows/ci-fast-gate.yml --output-json /tmp/anti-flake-policy-report.json`

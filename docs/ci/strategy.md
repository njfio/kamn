...
- Fast-gate review-freeze contract command:
...
- Deterministic changed-files command:
...
## Runtime Request-Response Schema Compatibility Contract Lane
### Request-Response Schema Compatibility Contract
- Entry commands:
...
- manual-hardened mode: manual
- Cost controls:
  - dry-run mode executes no nested request-response schema compatibility commands and emits deterministic `dry_run_no_commands_executed`.
  - run mode is explicit local-only and requires `KAMN_REQUEST_RESPONSE_SCHEMA_COMPATIBILITY_OPT_IN=1`.
  - run mode replays fixture pair projection without nested cargo command fan-out.
...
- Entry commands:
  - `bash scripts/runtime/run_dependency_local_heavy_deep_scan_lane.sh --profile baseline --mode dry-run --ci-fast-gate PASS --max-seconds 180 --output-json /tmp/dependency-local-heavy-deep-scan-baseline.json`
  ...
  - `cargo test -p kamn-core --test dependency_local_heavy_deep_scan_policy_contract -- --nocapture`
  - `cargo test -p kamn-core --test ci_strategy_docs doc_contains_dependency_local_heavy_deep_scan_policy_checker_contract -- --exact`
- ci-fast-gate mode: fast
- local-dev mode: local
- manual-hardened mode: manual
- Cost controls:
  - checker reads deterministic dry-run lane reports plus docs/workflow/ci-tools text markers only.
  - no local-heavy run-mode execution is required for checker pass/fail validation.
  - checker runtime remains bounded and low-cost for fast-gate usage.
- Deterministic checker markers:
  ...
  - `dependency_local_heavy_deep_scan_policy_runner_reason_codes_csv=dependency_local_heavy_deep_scan_profile_threshold_exceeded,dependency_local_heavy_deep_scan_runtime_budget_exceeded`
...
- Advisory workflow:
  ...
  - `supply_chain_advisory_repo_toolchain_file=rust-toolchain.toml`
  - `supply_chain_advisory_repo_toolchain_channel=1.88.0`
  - `supply_chain_advisory_builder_image=rust:1.88-bookworm`

...
- Local-heavy boundary policy:
  ...
  - local-only run mode remains explicit via `KAMN_LOCAL_FULL_STACK_INTEGRATION_OPT_IN=1`.
...
- Convergence and policy commands:
...
- Deterministic promotion and evidence markers:
...
- Regression policy:
  - missing/tampered lane-policy-source convergence linkage forces `NO-GO` (`Regression: #4259`).
...
- Deterministic checkpoints include:
  ...
  - provider-failure taxonomy remains deterministic in policy outputs:

...
## SQLite Crash-Restart Local-Heavy Lane Artifact Contract (Issue #4017)

Deterministic crash-restart local-heavy runner markers:
...
- `profile=restart -> restart_drill_status=verified`
- `profile=corruption -> corruption_drill_status=verified`
...
## Local-Heavy Capacity/Load Lane Artifact Contract (Issue #4004)

Deterministic local-heavy capacity/load runner markers:
...
- `local_heavy_capacity_load_required_profiles_csv=baseline,fault`

Profile projection contracts:

- `profile=baseline -> status=pass + final_decision=GO + profile_status=verified`
- `profile=fault -> status=fail + final_decision=NO-GO + profile_status=failed`

Deterministic throughput/latency/error markers:

- `profile=baseline -> throughput_tps=11250 + latency_p50_ms=92 + latency_p99_ms=360 + error_rate_bps=18`
- `profile=fault -> throughput_tps=8400 + latency_p50_ms=168 + latency_p99_ms=620 + error_rate_bps=145`
...
Lifecycle tamper governance uses low-cost CI dry-run artifacts and fails closed when lifecycle
integrity or release go/no-go marker parity drifts.

CI composition commands:
...
### Parallel Lane Topology Host-Cardinality Mapping Contracts (Issue #5380)

The live-postgres matrix contracts now codify explicit topology-id to unique-host cardinality
mapping rows so topology labels cannot drift between one-host and two-host expectations.

Deterministic topology-id host-cardinality mapping markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-pair-cardinality
coherence rows so host-pair labels and host-count expectations cannot drift apart.

Deterministic topology-id host-pair-cardinality coherence markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-mode-host-pair
coherence rows so host-mode labels and host-pair expectations cannot drift apart.

Deterministic topology-id host-mode-host-pair coherence markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-mode-host-pair-cardinality
coherence rows so host-mode labels, host-pair bindings, and host-count expectations cannot drift apart.

Deterministic topology-id host-mode-host-pair-cardinality coherence markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-mode-host-pair-lane-set-lane-count
coherence rows so host-mode labels, host-pair bindings, lane-set class, and lane-count expectations cannot drift apart.

Deterministic topology-id host/lane composite coherence markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-mode-host-pair-lane-set-lane-id-bundle
coherence rows so host-mode labels, host-pair bindings, lane-set class, and lane identity bundles cannot drift apart.

Deterministic topology-id host/lane-id bundle coherence markers:
...
The live-postgres matrix contracts now codify explicit topology-id to host-mode-host-pair-lane-set-lane-fingerprint-bundle
coherence rows so host-mode labels, host-pair bindings, lane-set class, and lane fingerprint bundles cannot drift apart.

Deterministic topology-id host/lane fingerprint-bundle coherence markers:
...
coherence rows so lane fingerprint-bundle integrity can be validated with compact deterministic hash markers.

Deterministic topology-id host/lane fingerprint-hash coherence markers:
...
### Parallel Lane Topology Host-Mode-Host-Pair-Lane-Set-Lane-Fingerprint-Hash Order-Normalization-Digest Contracts (Issue #5400)

The live-postgres matrix contracts now codify deterministic digest markers for canonical topology-id to
host-mode-host-pair-lane-set-lane-fingerprint-hash order-normalization rows so lightweight integrity checks can fail closed on row drift.

Deterministic topology-id host/lane fingerprint-hash order-normalization digest markers:

- `phase6_live_postgres_daemon_runtime_parallel_lane_topology_host_mode_host_pair_lane_set_lane_fingerprint_hash_order_normalization_digest_schema_version=kamn.runtime.daemon.phase6-live-postgres.parallel-lane-topology-host-mode-host-pair-lane-set-lane-fingerprint-hash-order-normalization-digest.v1`
- `phase6_live_postgres_daemon_runtime_parallel_lane_topology_host_mode_host_pair_lane_set_lane_fingerprint_hash_order_normalization_digest_rows_csv=distributed_label_parallel->distributed_label->node_alpha->node_beta->asymmetric_parallel->18ce08940c67c38e,same_host_parallel->same_host->node_alpha->node_alpha->symmetric_parallel->37e351d41d1e30ea`
- `phase6_live_postgres_daemon_runtime_parallel_lane_topology_host_mode_host_pair_lane_set_lane_fingerprint_hash_order_normalization_digest_rows_fnv1a64_hex=25b9729eaeb44fe9`
- `phase6_live_postgres_daemon_runtime_parallel_lane_topology_host_mode_host_pair_lane_set_lane_fingerprint_hash_order_normalization_digest_contract=topology_id_to_host_mode_host_pair_lane_set_lane_fingerprint_hash_rows_digest_must_remain_stable_under_order_normalization_and_permutations`

...
| T4 | Remaining data-layer surfaces (`data_layer_m0`-`data_layer_m11`, `data_layer_phase2_crypto_blind_index_pipeline`, `data_layer_postgres_execution_adapter`, `data_layer_postgres_repository_bridge`) | `crates/kamn-data-layer` | `kamn-data-layer` already owns extracted M1, PRD critical-scenario, shell-neutral policy, and M10/M11 helper seams, so expanding that same boundary next reduces a large cohesive monolith surface before later external-facing tranches depend on it. |

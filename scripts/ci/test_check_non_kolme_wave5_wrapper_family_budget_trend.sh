...
TREND_CHECKER="$ROOT_DIR/scripts/ci/check_non_kolme_wave5_wrapper_family_budget_trend.sh"
PYTHON_CHECKER="$ROOT_DIR/scripts/ci/kolme_wrapper_inventory_baseline.py"
THRESHOLD_FILE="$ROOT_DIR/fixtures/ci/non_kolme_wave5_wrapper_family_trend_thresholds.json"
BASELINE_FIXTURE="$ROOT_DIR/fixtures/ci/non_kolme_wave5_wrapper_family_baseline.json"
MATRIX_FIXTURE="$ROOT_DIR/fixtures/ci/non_kolme_wave5_wrapper_family_matrix.json"

TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

if [ ! -x "$TREND_CHECKER" ]; then
  echo "expected non-Kolme wave-5 trend checker wrapper to be executable" >&2
  exit 1
fi
...
if [ ! -f "$THRESHOLD_FILE" ]; then
  echo "expected non-Kolme wave-5 trend threshold fixture to exist" >&2
  exit 1
fi

if [ ! -f "$BASELINE_FIXTURE" ]; then
  echo "expected non-Kolme wave-5 baseline fixture to exist" >&2
  exit 1
fi

if [ ! -f "$MATRIX_FIXTURE" ]; then
  echo "expected non-Kolme wave-5 matrix fixture to exist" >&2
  exit 1
fi
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-5 trend checker to fail when total shell LOC delta exceeds threshold" >&2
  exit 1
fi
...
MUTATED_LANE_WRAPPER="$TMP_DIR/run_bridge_replay_redaction_contract_lane.sh"
cp "$MATRIX_FIXTURE" "$MUTATED_LANE_MATRIX"
cat >"$MUTATED_LANE_WRAPPER" <<'INNER'
#!/usr/bin/env bash
set -euo pipefail

printf 'mutated bridge replay redaction lane\n'
INNER
...
for lane in payload["lanes"]:
    if lane.get("lane_id") == "non_kolme.bridge.replay_redaction":
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-5 trend checker to fail when lane source_entry drifts from baseline" >&2
  exit 1
fi
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-5 trend checker to fail on stale baseline lane inventory" >&2
  exit 1
fi
...
echo "non-Kolme wave-5 wrapper-family budget trend checker tests passed."

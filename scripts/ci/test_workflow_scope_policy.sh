...
EXPECTED_GOVERNANCE_RATIO_ACTIVATION_BASE_SHA="d2c2fe1bbf68806cb153541f9dc192c81e7847cf"
...
if ! grep -Fq "GOVERNANCE_FEATURE_COMMIT_RATIO_MORATORIUM_BASE_SHA=$EXPECTED_GOVERNANCE_RATIO_ACTIVATION_BASE_SHA" "$ROOT_DIR/.ci/governance-feature-commit-ratio-moratorium.env"; then
  echo "expected governance/feature commit ratio moratorium env to anchor at the restored base-branch compliance head" >&2
  exit 1
fi
...
if ! grep -Fq "bash scripts/kolme/test_run_local_fork_smoke_evidence_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local-only Kolme fork smoke evidence command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi

if ! grep -Fq "bash scripts/kolme/test_run_local_kolme_api_probe_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local Kolme API probe command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi

if ! grep -Fq "bash scripts/kolme/test_run_local_kolme_api_smoke_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local-only Kolme API smoke command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi
...
if ! grep -Fq "bash scripts/kolme/test_run_local_kolme_fork_bootstrap_readiness_contract_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local fork bootstrap/readiness command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi

if ! grep -Fq "bash scripts/kolme/test_run_local_kamn_live_runtime_integration_contract_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local KAMN live runtime integration command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi
...
if ! grep -Fq "bash scripts/kolme/test_run_local_kolme_fork_rust_test_matrix_lane.sh" "$FAST_WORKFLOW"; then
  echo "expected local fork rust matrix lane command-surface tests in ci-fast-gate.yml" >&2
  exit 1
fi
...
if grep -Fq "bash scripts/kolme/run_local_kolme_api_probe_lane.sh --mode run" "$FAST_WORKFLOW"; then
  echo "expected local Kolme API probe run-mode lane to remain excluded from ci-fast-gate.yml" >&2
  exit 1
fi

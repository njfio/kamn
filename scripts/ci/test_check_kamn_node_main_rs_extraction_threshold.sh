...
if [ ! -x "$CHECKER" ]; then
  echo "expected kamn-node main.rs extraction-threshold checker wrapper to be executable" >&2
  exit 1
fi

if [ ! -f "$THRESHOLD_FILE" ]; then
  echo "expected kamn-node main.rs extraction-threshold fixture to exist" >&2
  exit 1
fi

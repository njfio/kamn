...
usage() {
  cat >&2 <<'USAGE'
Usage:
  check_kolme_wave<id>_wrapper_family_budget_trend.sh [checker-args...]
  check_kolme_wave_wrapper_family_budget_trend_impl.sh --wave-id <id> [checker-args...]
USAGE
}

if [[ "$SCRIPT_NAME" =~ ^check_kolme_wave([0-9]+)_wrapper_family_budget_trend\.sh$ ]]; then
...
if [ ! -f "$THRESHOLD_FILE" ]; then
  echo "expected Kolme wave-${WAVE_ID} trend threshold file to exist: $THRESHOLD_FILE" >&2
  exit 1
fi

exec python3 "$PYTHON_CHECKER" check \
  --trend-mode \

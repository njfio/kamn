...
if [ "$#" -ne 11 ]; then
  echo "usage: $0 <checker> <threshold-file> <resource-label> <threshold-schema> <exception-schema> <warn-reason-code> <fail-reason-code> <exception-applied-reason-code> <exception-expired-reason-code> <tracking-issue> <success-message>" >&2
  exit 64
fi
...
WARN_REASON_CODE="$6"
FAIL_REASON_CODE="$7"
EXCEPTION_APPLIED_REASON_CODE="$8"
EXCEPTION_EXPIRED_REASON_CODE="$9"
TRACKING_ISSUE="${10}"
SUCCESS_MESSAGE="${11}"
...
test_harness_require_executable "$CHECKER" "expected $RESOURCE_LABEL extraction-threshold checker wrapper to be executable"
test_harness_require_file "$THRESHOLD_FILE" "expected $RESOURCE_LABEL extraction-threshold fixture to exist"
...
grep -q "$WARN_REASON_CODE" "$TMP_DIR/warn.out"
...
if [ "$fail_code" -eq 0 ]; then
  echo "expected checker to fail when $RESOURCE_LABEL line count exceeds fail threshold without tracked exception" >&2
  exit 1
fi

grep -q '^status=fail$' "$TMP_DIR/fail.out"
grep -q '^policy_decision=NO-GO$' "$TMP_DIR/fail.out"
grep -q "$FAIL_REASON_CODE" "$TMP_DIR/fail.out"
...
grep -q "^reason_codes=$EXCEPTION_APPLIED_REASON_CODE$" "$TMP_DIR/exception-pass.out"
...
grep -q "$EXCEPTION_EXPIRED_REASON_CODE" "$TMP_DIR/exception-expired.out"

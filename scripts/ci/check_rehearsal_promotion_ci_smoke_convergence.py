#!/usr/bin/env python3
"""Validate low-cost CI smoke convergence for rehearsal/promotion governance."""
...
REPORT_SCHEMA_VERSION = "kamn.ci.rehearsal-promotion-ci-smoke-convergence-report.v1"
REASON_TAXONOMY_VERSION = (
    "kamn.ci.rehearsal-promotion-ci-smoke-convergence-reason-taxonomy.v1"
)
REASON_CODES_CSV = (
...
REHEARSAL_DEEP_LOCAL_HEAVY_COMMAND = (
    'bash "$ROOT_DIR/scripts/deploy/run_staging_rehearsal_deep_lane.sh"'
)
REHEARSAL_DEEP_WORKFLOW_COMMAND = (
    "bash scripts/deploy/run_staging_rehearsal_deep_lane.sh"
)
REHEARSAL_BUNDLE_WORKFLOW_COMMAND = (
    "bash scripts/deploy/test_generate_staging_rehearsal_bundle.sh"
)
REHEARSAL_CONTRACT_LANE_WORKFLOW_COMMAND = (
...
STRATEGY_REQUIRED_MARKERS = (
    ...
    "python3 scripts/ci/check_rehearsal_promotion_ci_smoke_convergence.py",
...
PLAN_REQUIRED_MARKERS = (
    ...
    "rehearsal_promotion_ci_smoke_max_seconds=120",
    "rehearsal_promotion_local_heavy_max_seconds=900",
    "check_rehearsal_promotion_ci_smoke_convergence.py --workflow-file .github/workflows/ci-fast-gate.yml --ci-tools-file scripts/ci/test_ci_tools.sh --strategy-doc docs/ci/strategy.md --plan-doc docs/plans/2026-02-14-production-service-next-steps.md --max-seconds 120 --output-json /tmp/rehearsal-promotion-ci-smoke-convergence-report.json",
    "test_check_rehearsal_promotion_ci_smoke_convergence.sh",
)
...
def main() -> int:
    ...
    if args.max_seconds > CI_SMOKE_MAX_SECONDS:
        raw_reason_codes.append("rehearsal_promotion_ci_smoke_seconds_exceeded")
    ...
    if workflow_text:
        if REHEARSAL_BUNDLE_WORKFLOW_COMMAND not in workflow_text:
            raw_reason_codes.append("rehearsal_bundle_ci_smoke_composition_missing")
        if REHEARSAL_CONTRACT_LANE_WORKFLOW_COMMAND not in workflow_text:
            raw_reason_codes.append("rehearsal_contract_lane_ci_smoke_composition_missing")
        if REHEARSAL_DEEP_WORKFLOW_COMMAND in workflow_text:
            raw_reason_codes.append("ci_fast_gate_rehearsal_deep_lane_not_excluded")

    if fast_mode_block:
        if REHEARSAL_DEEP_LOCAL_HEAVY_COMMAND in fast_mode_block:
            raw_reason_codes.append("rehearsal_deep_lane_command_leaked_in_fast_mode")
    ...
    report = {
        ...
        "rehearsal_promotion_ci_smoke_convergence_status": convergence_status,
        "rehearsal_promotion_ci_smoke_max_seconds": CI_SMOKE_MAX_SECONDS,
        "rehearsal_promotion_local_heavy_max_seconds": LOCAL_HEAVY_MAX_SECONDS,
        "rehearsal_promotion_ci_smoke_lane_cost_profile": "low",
        "rehearsal_promotion_local_heavy_execution_mode": "opt_in",
        "inputs": {
    ...
    print(f"rehearsal_promotion_ci_smoke_convergence_status={convergence_status}")
    print(f"rehearsal_promotion_ci_smoke_max_seconds={CI_SMOKE_MAX_SECONDS}")
    print(f"rehearsal_promotion_local_heavy_max_seconds={LOCAL_HEAVY_MAX_SECONDS}")
    print("rehearsal_promotion_ci_smoke_lane_cost_profile=low")
    print("rehearsal_promotion_local_heavy_execution_mode=opt_in")

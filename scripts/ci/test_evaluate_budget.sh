...
run_expect_status fast_warn warn --budget-file "$BUDGET_FILE" --lane fast-gate --elapsed-seconds 1005 --test-scope full --changed-files 10 --job-count 1
run_expect_failure fast_fail --budget-file "$BUDGET_FILE" --lane fast-gate --elapsed-seconds 1101 --test-scope full --changed-files 12 --job-count 1
run_expect_status fast_retry_grace_pass pass --budget-file "$BUDGET_FILE" --lane fast-gate --elapsed-seconds 1193 --test-scope full --changed-files 33 --job-count 1 --retry-used true
run_expect_failure fast_retry_grace_fail --budget-file "$BUDGET_FILE" --lane fast-gate --elapsed-seconds 1401 --test-scope full --changed-files 33 --job-count 1 --retry-used true

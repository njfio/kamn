...
if [ "$total_shell_loc" -gt 60 ]; then
  echo "expected tranche-1 migrated shell LOC to stay at or below 60 lines, got ${total_shell_loc}" >&2
  exit 1
fi

echo "Kolme tranche-1 manifest migration contract lane checks passed."

...
if grep -Fq "bash scripts/runtime/validate_local_observability_scrape_live.sh --mode run" "$FAST_WORKFLOW"; then
...
if printf '%s\n' "$fast_mode_block" | grep -Fq 'bash "$ROOT_DIR/scripts/runtime/test_validate_local_observability_scrape_live_contract_lane.sh"'; then
  echo "expected local observability scrape contract lane to remain excluded from ci-tools fast mode" >&2
  exit 1
fi

for required_command in \
  'bash "$ROOT_DIR/scripts/runtime/test_check_local_observability_scrape_live_policy.sh"' \
  'bash "$ROOT_DIR/scripts/runtime/test_validate_local_observability_scrape_live_contract_lane.sh"'; do
  if ! grep -Fq "$required_command" "$CI_TOOLS_SCRIPT"; then
    echo "expected ci-tools regression lane to include local observability scrape command: $required_command" >&2
    exit 1
  fi
done
...
echo "local observability scrape CI exclusion policy tests passed."

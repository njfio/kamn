#!/usr/bin/env python3
"""Validate low-cost CI smoke convergence for websocket-session governance."""
...
REPORT_SCHEMA_VERSION = "kamn.ci.websocket-session-ci-smoke-convergence-report.v1"
REASON_TAXONOMY_VERSION = "kamn.ci.websocket-session-ci-smoke-convergence-reason-taxonomy.v1"
REASON_CODES_CSV = (
    "workflow_file_missing,"
    "ci_tools_file_missing,"
    "strategy_doc_missing,"
    "plan_doc_missing,"
    "websocket_validate_ci_smoke_composition_missing,"
    "websocket_policy_ci_smoke_composition_missing,"
    "websocket_contract_ci_smoke_composition_missing,"
...
WEBSOCKET_VALIDATE_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_validate_service_api_websocket_live.sh"'
)
WEBSOCKET_POLICY_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_check_service_api_websocket_live_policy.sh"'
)
WEBSOCKET_CONTRACT_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_validate_service_api_websocket_live_contract_lane.sh"'
)
WEBSOCKET_SESSION_DRILL_LOCAL_HEAVY_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/validate_service_api_websocket_live_contract_lane.sh"'
)
WEBSOCKET_SESSION_DRILL_WORKFLOW_COMMAND = (
    "bash scripts/runtime/validate_service_api_websocket_live_contract_lane.sh"
)

STRATEGY_REQUIRED_MARKERS = (
    "### Websocket Session CI smoke convergence governance",
    "python3 scripts/ci/check_websocket_session_ci_smoke_convergence.py",
...
def main() -> int:
    ...
    if args.max_seconds > CI_SMOKE_MAX_SECONDS:
        raw_reason_codes.append("websocket_session_ci_smoke_seconds_exceeded")
    ...
    if workflow_text and WEBSOCKET_SESSION_DRILL_WORKFLOW_COMMAND in workflow_text:
        raw_reason_codes.append("ci_fast_gate_websocket_session_drill_not_excluded")

    if fast_mode_block:
        if WEBSOCKET_VALIDATE_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("websocket_validate_ci_smoke_composition_missing")
        if WEBSOCKET_POLICY_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("websocket_policy_ci_smoke_composition_missing")
        if WEBSOCKET_CONTRACT_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("websocket_contract_ci_smoke_composition_missing")
        if WEBSOCKET_SESSION_DRILL_LOCAL_HEAVY_COMMAND in fast_mode_block:
            raw_reason_codes.append("websocket_session_drill_run_command_leaked_in_fast_mode")

    if strategy_text:
        if any(marker not in strategy_text for marker in STRATEGY_REQUIRED_MARKERS):
            raw_reason_codes.append("ci_strategy_websocket_session_convergence_markers_missing")

    if plan_text:
        if any(marker not in plan_text for marker in PLAN_REQUIRED_MARKERS):
            raw_reason_codes.append("production_plan_websocket_session_convergence_markers_missing")
    ...
    report = {
        ...
        "websocket_session_ci_smoke_convergence_status": convergence_status,
        "websocket_session_ci_smoke_max_seconds": CI_SMOKE_MAX_SECONDS,
        "websocket_session_local_heavy_max_seconds": LOCAL_HEAVY_MAX_SECONDS,
        "websocket_session_ci_smoke_lane_cost_profile": "low",
        "websocket_session_local_heavy_execution_mode": "opt_in",
        "inputs": {
    ...
    print(f"websocket_session_ci_smoke_convergence_status={convergence_status}")
    print(f"websocket_session_ci_smoke_max_seconds={CI_SMOKE_MAX_SECONDS}")
    print(f"websocket_session_local_heavy_max_seconds={LOCAL_HEAVY_MAX_SECONDS}")
    print("websocket_session_ci_smoke_lane_cost_profile=low")
    print("websocket_session_local_heavy_execution_mode=opt_in")

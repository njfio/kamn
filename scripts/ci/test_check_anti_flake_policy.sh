...
if report.get("reason_taxonomy_version") != "kamn.ci.merge-gate-reliability-reason-taxonomy.v1":
...
if ! printf '%s\n' "$mismatch_output" | grep -q '^anti_flake_policy_reason_codes=expected_final_decision_mismatch,no_active_flaky_entries$'; then

#!/usr/bin/env python3
"""Enforce kamn-node runtime_orchestration.rs extraction threshold growth policy."""
...
THRESHOLD_SCHEMA = "kamn.ci.kamn-node-runtime-orchestration-rs-extraction-thresholds.v1"
EXCEPTION_SCHEMA = "kamn.ci.kamn-node-runtime-orchestration-rs-extraction-threshold-exception.v1"
OUTPUT_SCHEMA = "kamn.ci.kamn-node-runtime-orchestration-rs-extraction-threshold-report.v1"


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check kamn-node runtime_orchestration.rs extraction threshold policy."
    )
    parser.add_argument(
        "--source-file",
        default="crates/kamn-node/src/runtime_orchestration.rs",
        help="Path to runtime_orchestration.rs source file.",
    )
    parser.add_argument(
        "--threshold-file",
        default="fixtures/ci/kamn_node_runtime_orchestration_rs_extraction_thresholds.json",
...
def parse_exception(path: Path) -> tuple[str, date, int]:
    try:
        ...
        raise ValueError(f"runtime_orchestration_rs_threshold_exception_json_invalid:exception JSON invalid: {error}") from error
    if payload.get("schema_version") != EXCEPTION_SCHEMA:
        raise ValueError("runtime_orchestration_rs_threshold_exception_schema_mismatch:unexpected exception schema")
    tracking_issue = payload.get("tracking_issue")
    if not isinstance(tracking_issue, str) or not tracking_issue.strip():
        raise ValueError(
            "runtime_orchestration_rs_threshold_exception_tracking_issue_invalid:tracking_issue must be non-empty string"
        )
    expires_on = payload.get("expires_on")
    if not isinstance(expires_on, str) or not expires_on.strip():
        raise ValueError(
            "runtime_orchestration_rs_threshold_exception_expires_on_invalid:expires_on must be non-empty string"
        )
    try:
        expiry_date = datetime.strptime(expires_on, "%Y-%m-%d").date()
    except ValueError as error:
        raise ValueError(
            "runtime_orchestration_rs_threshold_exception_expires_on_format_invalid:expires_on must be YYYY-MM-DD"
        ) from error
    if expiry_date < date.today():
        raise ValueError("runtime_orchestration_rs_threshold_exception_expired:tracked exception has expired")
    max_line_count = payload.get("max_line_count")
    if not isinstance(max_line_count, int) or max_line_count <= 0:
        raise ValueError(
            "runtime_orchestration_rs_threshold_exception_max_line_count_invalid:max_line_count must be positive int"
        )
...
def main(argv: list[str]) -> int:
    ...
    if line_count <= fail_line_count:
        return print_result(
            ...
            exception_status="not-required",
            exception_tracking_issue="none",
            remediation="extract high-churn runtime_orchestration.rs logic into dedicated modules before fail threshold",
            output_json=args.output_json,
        )
    ...
    exception_tracking_issue = "none"
    remediation = (
        "extract high-churn runtime_orchestration.rs logic below fail threshold or provide a short-lived tracked exception"
    )

    if args.exception_file:
        exception_file = (root_dir / args.exception_file).resolve()
        if not exception_file.is_file():
            reason_codes.append("runtime_orchestration_rs_threshold_exception_file_not_found")
            ...
            try:
                ...
                if line_count > max_line_count:
                    reason_codes.append("runtime_orchestration_rs_threshold_exception_cap_exceeded")
                    exception_status = "cap-exceeded"
                    remediation = "reduce runtime_orchestration.rs line count under exception max_line_count or refresh exception policy"
                else:
                    return print_result(
                        status="warn",
                        policy_decision="WARN",
                        ...
                        exception_tracking_issue=exception_tracking_issue,
                        remediation="tracked exception applied; reduce runtime_orchestration.rs below fail threshold before exception expiry",
                        output_json=args.output_json,
                    )

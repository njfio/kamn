#!/usr/bin/env python3
"""Validate low-cost CI smoke convergence for failover drift governance."""
...
REPORT_SCHEMA_VERSION = "kamn.ci.failover-drift-ci-smoke-convergence-report.v1"
REASON_TAXONOMY_VERSION = "kamn.ci.failover-drift-ci-smoke-convergence-reason-taxonomy.v1"
REASON_CODES_CSV = (
    ...
    "failover_deep_lane_guard_ci_smoke_composition_missing,"
    ...
    "ci_strategy_failover_convergence_markers_missing,"
    "production_plan_failover_convergence_markers_missing,"
...
SELECTOR_CI_SMOKE_COMMAND = 'bash "$ROOT_DIR/scripts/runtime/test_select_failover_sync_drill_lane.sh"'
PREFLIGHT_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_run_failover_sync_drill_preflight_contract_lane.sh"'
)
DEEP_GUARD_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_run_failover_sync_drill_deep_lane.sh"'
)
SUITE_CI_SMOKE_COMMAND = 'bash "$ROOT_DIR/scripts/runtime/test_run_failover_sync_drill_suite.sh"'
DEEP_LOCAL_HEAVY_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/run_failover_sync_drill_deep_lane.sh"'
)
DEEP_WORKFLOW_COMMAND = "bash scripts/runtime/run_failover_sync_drill_deep_lane.sh"

STRATEGY_REQUIRED_MARKERS = (
    "### Failover Drift CI smoke convergence governance",
    "python3 scripts/ci/check_failover_drift_ci_smoke_convergence.py",
...
def main() -> int:
    ...
    if args.max_seconds > CI_SMOKE_MAX_SECONDS:
        raw_reason_codes.append("failover_drift_ci_smoke_seconds_exceeded")
    ...
    if workflow_text and DEEP_WORKFLOW_COMMAND in workflow_text:
        raw_reason_codes.append("ci_fast_gate_failover_deep_lane_not_excluded")

    if fast_mode_block:
        if SELECTOR_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_selector_ci_smoke_composition_missing")
        if PREFLIGHT_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_preflight_ci_smoke_composition_missing")
        if DEEP_GUARD_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_deep_lane_guard_ci_smoke_composition_missing")
        if SUITE_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_suite_ci_smoke_composition_missing")
        if DEEP_LOCAL_HEAVY_COMMAND in fast_mode_block:
            raw_reason_codes.append("failover_deep_lane_run_command_leaked_in_fast_mode")

    if strategy_text:
        if any(marker not in strategy_text for marker in STRATEGY_REQUIRED_MARKERS):
            raw_reason_codes.append("ci_strategy_failover_convergence_markers_missing")

    if plan_text:
        if any(marker not in plan_text for marker in PLAN_REQUIRED_MARKERS):
            raw_reason_codes.append("production_plan_failover_convergence_markers_missing")
    ...
    report = {
        ...
        "failover_drift_ci_smoke_convergence_status": convergence_status,
        "failover_drift_ci_smoke_max_seconds": CI_SMOKE_MAX_SECONDS,
        "failover_drift_local_heavy_max_seconds": LOCAL_HEAVY_MAX_SECONDS,
        "failover_drift_ci_smoke_lane_cost_profile": "low",
        "failover_drift_local_heavy_execution_mode": "opt_in",
        "inputs": {
    ...
    print(f"failover_drift_ci_smoke_convergence_status={convergence_status}")
    print(f"failover_drift_ci_smoke_max_seconds={CI_SMOKE_MAX_SECONDS}")
    print(f"failover_drift_local_heavy_max_seconds={LOCAL_HEAVY_MAX_SECONDS}")
    print("failover_drift_ci_smoke_lane_cost_profile=low")
    print("failover_drift_local_heavy_execution_mode=opt_in")

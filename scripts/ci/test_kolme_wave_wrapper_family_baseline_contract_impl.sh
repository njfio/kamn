...
usage() {
  cat >&2 <<'USAGE'
Usage: test_kolme_wave_wrapper_family_baseline_contract_impl.sh --wave-id <id>
USAGE
}
...
wave_id = sys.argv[2]
payload = json.loads(path.read_text(encoding="utf-8"))
payload["lanes"][0]["source_entry"] = f"scripts/ci/run_missing_kolme_wave{wave_id}_wrapper.sh"
path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
PY

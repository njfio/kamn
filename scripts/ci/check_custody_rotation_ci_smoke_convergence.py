#!/usr/bin/env python3
"""Validate low-cost CI smoke convergence for custody/rotation governance."""
...
REPORT_SCHEMA_VERSION = "kamn.ci.custody-rotation-ci-smoke-convergence-report.v1"
REASON_TAXONOMY_VERSION = (
    "kamn.ci.custody-rotation-ci-smoke-convergence-reason-taxonomy.v1"
)
REASON_CODES_CSV = (
...
def main() -> int:
    ...
    if args.max_seconds > CI_SMOKE_MAX_SECONDS:
        raw_reason_codes.append("custody_rotation_ci_smoke_seconds_exceeded")
    ...
    if workflow_text and FAILOVER_DEEP_WORKFLOW_COMMAND in workflow_text:
        raw_reason_codes.append("ci_fast_gate_failover_deep_lane_not_excluded")

    if fast_mode_block:
        if (
            CUSTODY_ROTATION_SMOKE_CONTRACT_CI_SMOKE_COMMAND
            not in fast_mode_block
        ):
            raw_reason_codes.append(
                "custody_rotation_smoke_contract_ci_smoke_composition_missing"
            )
        if FAILOVER_PREFLIGHT_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_preflight_ci_smoke_composition_missing")
        if FAILOVER_SUITE_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append("failover_suite_ci_smoke_composition_missing")
        if FAILOVER_DEEP_LOCAL_HEAVY_COMMAND in fast_mode_block:
            raw_reason_codes.append("failover_deep_lane_command_leaked_in_fast_mode")
    ...
    report = {
        ...
        "custody_rotation_ci_smoke_convergence_status": convergence_status,
        "custody_rotation_ci_smoke_max_seconds": CI_SMOKE_MAX_SECONDS,
        "custody_rotation_local_heavy_max_seconds": LOCAL_HEAVY_MAX_SECONDS,
        "custody_rotation_ci_smoke_lane_cost_profile": "low",
        "custody_rotation_local_heavy_execution_mode": "opt_in",
        "inputs": {
    ...
    print(f"custody_rotation_ci_smoke_convergence_status={convergence_status}")
    print(f"custody_rotation_ci_smoke_max_seconds={CI_SMOKE_MAX_SECONDS}")
    print(f"custody_rotation_local_heavy_max_seconds={LOCAL_HEAVY_MAX_SECONDS}")
    print("custody_rotation_ci_smoke_lane_cost_profile=low")
    print("custody_rotation_local_heavy_execution_mode=opt_in")

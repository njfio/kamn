...
if [ "$total_shell_loc" -gt 120 ]; then
  echo "expected profile+self-test+portability migrated shell LOC to stay at or below 120 lines, got ${total_shell_loc}" >&2
  exit 1
fi

echo "Kolme profile+self-test+portability manifest migration contract lane checks passed."

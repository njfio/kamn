...
if [ ! -f "$MATRIX_FIXTURE" ]; then
  echo "expected non-Kolme wave-6 wrapper-family matrix fixture to exist" >&2
  exit 1
fi

if [ ! -f "$BASELINE_FIXTURE" ]; then
  echo "expected non-Kolme wave-6 wrapper-family baseline fixture to exist" >&2
  exit 1
fi
...
if payload["symlink_wrapper_count"] != 4:
    raise SystemExit("expected non-Kolme wave-6 symlink_wrapper_count to be 4")
if payload["regular_file_wrapper_count"] != 0:
    raise SystemExit("expected non-Kolme wave-6 regular_file_wrapper_count to be 0")
if payload["total_shell_loc"] != 4:
    raise SystemExit("expected non-Kolme wave-6 total_shell_loc to be 4")
PY
...
if baseline != generated:
    raise SystemExit("non-Kolme wave-6 generated baseline fixture drift detected")
PY
...
if bash "$CHECK_SCRIPT" \
  ...
  echo "expected non-Kolme wave-6 baseline checker to fail on shell_loc drift" >&2
  exit 1
fi
...
path = Path(sys.argv[1])
payload = json.loads(path.read_text(encoding="utf-8"))
payload["lanes"][0]["source_entry"] = "scripts/bridge/run_missing_wave6_wrapper.sh"
path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
PY
...
if bash "$CHECK_SCRIPT" \
  ...
  echo "expected non-Kolme wave-6 baseline checker to fail when matrix source_entry wrapper is missing" >&2
  exit 1
fi
grep -q 'lane wrapper path does not exist' "$TMP_DIR/check-mutated-matrix.out"

echo "non-Kolme wave-6 wrapper-family baseline contract tests passed."

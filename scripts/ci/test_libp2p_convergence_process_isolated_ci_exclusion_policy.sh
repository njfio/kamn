...
if grep -Fq "bash scripts/runtime/validate_libp2p_convergence_process_isolated_live.sh --mode run" "$FAST_WORKFLOW"; then
  echo "expected process-isolated convergence run-mode lane to remain excluded from ci-fast-gate.yml" >&2
  exit 1
fi
...
if ! grep -Fq "process-isolated convergence run-mode commands remain excluded from ci-fast-gate and ci-tools fast mode." "$STRATEGY_DOC"; then

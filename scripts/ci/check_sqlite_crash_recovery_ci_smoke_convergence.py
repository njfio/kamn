#!/usr/bin/env python3
"""Validate low-cost CI smoke convergence for sqlite crash-recovery replay integrity."""
...
REPORT_SCHEMA_VERSION = "kamn.ci.sqlite-crash-recovery-ci-smoke-convergence-report.v1"
REASON_TAXONOMY_VERSION = (
    "kamn.ci.sqlite-crash-recovery-ci-smoke-convergence-reason-taxonomy.v1"
)
REASON_CODES_CSV = (
...
SQLITE_VALIDATE_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_validate_sqlite_crash_recovery_live.sh"'
)
SQLITE_POLICY_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_check_sqlite_crash_recovery_live_policy.sh"'
)
SQLITE_CONTRACT_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_validate_sqlite_crash_recovery_live_contract_lane.sh"'
)
SQLITE_EVIDENCE_CI_SMOKE_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/test_check_sqlite_crash_recovery_live_evidence_convergence.sh"'
)

SQLITE_RUN_MODE_LOCAL_HEAVY_COMMAND = (
    'bash "$ROOT_DIR/scripts/runtime/validate_sqlite_crash_recovery_live.sh" --mode run'
)
SQLITE_RUN_MODE_WORKFLOW_COMMAND = (
    "bash scripts/runtime/validate_sqlite_crash_recovery_live.sh --mode run"
)
...
def main() -> int:
    ...
    if args.max_seconds > CI_SMOKE_MAX_SECONDS:
        raw_reason_codes.append("sqlite_crash_recovery_ci_smoke_seconds_exceeded")
    ...
    if workflow_text and SQLITE_RUN_MODE_WORKFLOW_COMMAND in workflow_text:
        raw_reason_codes.append("ci_fast_gate_sqlite_crash_recovery_run_mode_not_excluded")

    if fast_mode_block:
        if SQLITE_VALIDATE_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append(
                "sqlite_crash_recovery_validate_ci_smoke_composition_missing"
            )
        if SQLITE_POLICY_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append(
                "sqlite_crash_recovery_policy_ci_smoke_composition_missing"
            )
        if SQLITE_CONTRACT_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append(
                "sqlite_crash_recovery_contract_lane_ci_smoke_composition_missing"
            )
        if SQLITE_EVIDENCE_CI_SMOKE_COMMAND not in fast_mode_block:
            raw_reason_codes.append(
                "sqlite_crash_recovery_evidence_ci_smoke_composition_missing"
            )
        if SQLITE_RUN_MODE_LOCAL_HEAVY_COMMAND in fast_mode_block:
            raw_reason_codes.append(
                "sqlite_crash_recovery_run_mode_command_leaked_in_fast_mode"
            )
    ...
    report = {
        ...
        "sqlite_crash_recovery_ci_smoke_convergence_status": convergence_status,
        "sqlite_crash_recovery_ci_smoke_max_seconds": CI_SMOKE_MAX_SECONDS,
        "sqlite_crash_recovery_local_heavy_max_seconds": LOCAL_HEAVY_MAX_SECONDS,
        "sqlite_crash_recovery_ci_smoke_lane_cost_profile": "low",
        "sqlite_crash_recovery_local_heavy_execution_mode": "opt_in",
        "inputs": {
    ...
    print(f"sqlite_crash_recovery_ci_smoke_convergence_status={convergence_status}")
    print(f"sqlite_crash_recovery_ci_smoke_max_seconds={CI_SMOKE_MAX_SECONDS}")
    print(f"sqlite_crash_recovery_local_heavy_max_seconds={LOCAL_HEAVY_MAX_SECONDS}")
    print("sqlite_crash_recovery_ci_smoke_lane_cost_profile=low")
    print("sqlite_crash_recovery_local_heavy_execution_mode=opt_in")

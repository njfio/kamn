...
TREND_CHECKER="$ROOT_DIR/scripts/ci/check_non_kolme_wave18_wrapper_family_budget_trend.sh"
PYTHON_CHECKER="$ROOT_DIR/scripts/ci/kolme_wrapper_inventory_baseline.py"
THRESHOLD_FILE="$ROOT_DIR/fixtures/ci/non_kolme_wave18_wrapper_family_trend_thresholds.json"
BASELINE_FIXTURE="$ROOT_DIR/fixtures/ci/non_kolme_wave18_wrapper_family_baseline.json"
MATRIX_FIXTURE="$ROOT_DIR/fixtures/ci/non_kolme_wave18_wrapper_family_matrix.json"

TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

if [ ! -x "$TREND_CHECKER" ]; then
  echo "expected non-Kolme wave-18 trend checker wrapper to be executable" >&2
  exit 1
fi
...
if [ ! -f "$THRESHOLD_FILE" ]; then
  echo "expected non-Kolme wave-18 trend threshold fixture to exist" >&2
  exit 1
fi

if [ ! -f "$BASELINE_FIXTURE" ]; then
  echo "expected non-Kolme wave-18 baseline fixture to exist" >&2
  exit 1
fi

if [ ! -f "$MATRIX_FIXTURE" ]; then
  echo "expected non-Kolme wave-18 matrix fixture to exist" >&2
  exit 1
fi
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-18 trend checker to fail when total shell LOC delta exceeds threshold" >&2
  exit 1
fi
...
MUTATED_LANE_WRAPPER="$TMP_DIR/run_deployment_slo_rollback_contract_lane.sh"
cp "$MATRIX_FIXTURE" "$MUTATED_LANE_MATRIX"
cat >"$MUTATED_LANE_WRAPPER" <<'INNER'
#!/usr/bin/env bash
set -euo pipefail

printf 'mutated deploy slo rollback lane\n'
INNER
...
for lane in payload["lanes"]:
    if lane.get("lane_id") == "non_kolme.deploy.deployment_slo_rollback":
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-18 trend checker to fail when lane source_entry drifts from baseline" >&2
  exit 1
fi
...
baseline_path = Path(sys.argv[1])
payload = json.loads(baseline_path.read_text(encoding="utf-8"))
payload["lanes"][0]["lane_id"] = "non_kolme.deploy.deployment_slo_rollback_stale"
...
if bash "$TREND_CHECKER" \
  ...
  echo "expected non-Kolme wave-18 trend checker to fail on stale baseline lane inventory" >&2
  exit 1
fi
...
echo "non-Kolme wave-18 wrapper-family budget trend checker tests passed."

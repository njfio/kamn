...
{
  "schema_version": "kamn.ci.test-harness-loc-report.v1",
  "harness_script_count": 25,
  "harness_shell_line_total": 250
}
EOF_REPORT
...
python3 - "$WITHIN_POLICY" "$EXCEEDED_POLICY" "$OUTPUT_JSON" <<'PY'
import json
import sys
...
require_equal(within, "trend_status", "not_configured", "within policy")
if within.get("reason_codes") not in ([], None):
    raise SystemExit("within policy: expected no reason_codes")

require_equal(exceeded, "status", "ok", "exceeded policy")
require_equal(exceeded, "soft_budget_status", "exceeded", "exceeded policy")
require_equal(exceeded, "policy_decision", "WARN", "exceeded policy")
require_equal(exceeded, "trend_status", "not_configured", "exceeded policy")
require_reason(exceeded, "harness_script_count_soft_max_exceeded", "exceeded policy")
require_reason(exceeded, "harness_shell_line_total_soft_max_exceeded", "exceeded policy")

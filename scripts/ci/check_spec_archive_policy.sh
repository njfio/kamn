...
REASON_CODES_CSV="spec_archive_entry_missing_file,spec_archive_output_json_required,spec_archive_output_write_failed,spec_archive_pointer_invalid,spec_archive_pointer_missing,spec_archive_root_missing,spec_archive_status_not_implemented"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --output-json)
      ...
      ;;
    *)
      emit_result "fail" "NO-GO" "spec_archive_root_missing" "unknown" "unknown"
      exit 1
      ;;
  esac
done
...
if [[ -z "$output_json" ]]; then
  emit_result "fail" "NO-GO" "spec_archive_output_json_required" "unknown" "unknown"
  exit 1
fi

archive_root="$repo_root/specs/archive"
if [[ ! -d "$archive_root" ]]; then
  emit_result "fail" "NO-GO" "spec_archive_root_missing" "0" "0"
  exit 1
fi
...
if ! cp "$tmp_report" "$output_json"; then
  emit_result "fail" "NO-GO" "spec_archive_output_write_failed" "$archived_issue_count" "$pointer_count"
  exit 1
fi

emit_result "$status" "$final_decision" "$reason_codes" "$archived_issue_count" "$pointer_count"

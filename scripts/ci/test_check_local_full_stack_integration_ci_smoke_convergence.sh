...
CHECKER="$ROOT_DIR/scripts/ci/check_local_full_stack_integration_ci_smoke_convergence.py"
...
test_harness_require_executable "$CHECKER" "expected local full-stack CI smoke convergence checker to be executable"
...
if ! python3 "$CHECKER" \
  ...
  cat "$safe_log" >&2
  echo "expected repository baseline to satisfy local full-stack CI smoke checker" >&2
  exit 1
fi
if ! grep -Fq "status=pass" "$safe_log"; then
  cat "$safe_log" >&2
  echo "expected local full-stack checker status=pass for repository baseline" >&2
  exit 1
fi
if ! grep -Fq "final_decision=GO" "$safe_log"; then
  cat "$safe_log" >&2
  echo "expected local full-stack checker final_decision=GO for repository baseline" >&2
  exit 1
fi
if ! grep -Fq "local_full_stack_ci_smoke_convergence_status=verified" "$safe_log"; then
  cat "$safe_log" >&2
  echo "expected local full-stack checker verified marker for repository baseline" >&2
  exit 1
fi
if ! grep -Fq "reason_codes_value=none" "$safe_log"; then
  cat "$safe_log" >&2
  echo "expected local full-stack checker reason_codes_value=none for repository baseline" >&2
  exit 1
fi
if ! grep -Fq '"final_decision": "GO"' "$safe_report"; then
  cat "$safe_report" >&2
  echo "expected local full-stack checker json final_decision GO for repository baseline" >&2
  exit 1
fi

missing_policy_ci_tools="$(mktemp)"
grep -Fv 'bash "$ROOT_DIR/scripts/runtime/test_check_local_full_stack_integration_live_policy.sh"' "$CI_TOOLS_SCRIPT" >"$missing_policy_ci_tools"
...
if ! grep -Fq "local_full_stack_policy_ci_smoke_composition_missing" "$missing_policy_log"; then
  cat "$missing_policy_log" >&2
  echo "expected missing local full-stack policy smoke command fixture to emit deterministic reason code" >&2
  exit 1
fi
...
awk '
  BEGIN { inserted = 0 }
  {
    if (!inserted && $0 ~ /echo "Fast-mode CI tool regression tests passed\."/ ) {
      print "  bash \"$ROOT_DIR/scripts/runtime/validate_local_full_stack_integration_live.sh\" --mode run --ci-fast-gate FAIL --output-json /tmp/local-full-stack-integration-summary.json"
      inserted = 1
    }
    print $0
  }
...
if ! grep -Fq "local_full_stack_run_mode_command_leaked_in_fast_mode" "$leaked_run_mode_log"; then
  cat "$leaked_run_mode_log" >&2
  echo "expected leaked local full-stack run-mode command fixture to emit deterministic leakage reason code" >&2
  exit 1
fi
...
cat >>"$leaked_workflow" <<'YAML'
      - name: Leaked local full-stack run-mode fixture
        run: bash scripts/runtime/validate_local_full_stack_integration_live.sh --mode run --ci-fast-gate FAIL --output-json /tmp/local-full-stack-integration-summary.json
YAML
...
if ! grep -Fq "ci_fast_gate_local_full_stack_run_mode_not_excluded" "$leaked_workflow_log"; then
  cat "$leaked_workflow_log" >&2
  echo "expected leaked local full-stack run-mode workflow fixture to emit deterministic exclusion reason code" >&2
  exit 1
fi

strategy_drift_doc="$(mktemp)"
grep -Fv "### Local Full-Stack Integration CI smoke convergence governance" "$STRATEGY_DOC" >"$strategy_drift_doc"
...
if ! grep -Fq "ci_strategy_local_full_stack_convergence_markers_missing" "$strategy_drift_log"; then
  cat "$strategy_drift_log" >&2
...
if ! grep -Fq "local_full_stack_ci_smoke_seconds_exceeded" "$budget_overflow_log"; then
  cat "$budget_overflow_log" >&2

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture",
        default=str(ROOT_DIR / "fixtures/compliance_soc2/control_evidence_replay_cases.json"),
    )
...
def run_generator(case: Dict[str, Any], bundle_path: Path) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        str(case["control_id"]),
        "--audit-period-start",
        str(case["audit_period_start"]),
        "--audit-period-end",
        str(case["audit_period_end"]),
        "--collector-did",
        str(case["collector_did"]),
        "--evidence-uri",
        str(case["evidence_uri"]),
        "--evidence-sha256",
        str(case["evidence_sha256"]),
        "--tamper-check",
        str(case["tamper_check"]),
        "--completeness-check",
        str(case["completeness_check"]),
        "--ci-fast-gate",
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema_version": "kamn.compliance.soc2-control-evidence.replay-matrix.v1",
            "status": status,

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture",
        default=str(ROOT_DIR / "fixtures/compliance_dsar/legal_hold_precedence_cases.json"),
    )
...
def run_generator(case: Dict[str, Any], bundle_path: Path) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        "--request-type",
        str(case["request_type"]),
        "--legal-hold-active",
        bool_value(case["legal_hold_active"]),
        "--retention-expired",
        bool_value(case["retention_expired"]),
        "--evidence-complete",
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema_version": "kamn.compliance.dsar-legal-hold.replay-matrix.v1",
            "status": status,

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture",
        default=str(ROOT_DIR / "fixtures/governance_stake_slash/risk_threshold_cases.json"),
    )
...
def run_generator(case: Dict[str, Any], bundle_path: Path) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        str(bundle_path),
        "--proposal-id",
        str(case["proposal_id"]),
        "--simulation-hash",
        str(case["simulation_hash"]),
        "--stake-at-risk-bps",
        str(case["stake_at_risk_bps"]),
        "--max-stake-at-risk-bps",
        str(case["max_stake_at_risk_bps"]),
        "--slash-probability-bps",
        str(case["slash_probability_bps"]),
        "--max-slash-probability-bps",
        str(case["max_slash_probability_bps"]),
        "--validator-churn-bps",
        str(case["validator_churn_bps"]),
        "--max-validator-churn-bps",
        str(case["max_validator_churn_bps"]),
        "--quorum-safety-margin-bps",
        str(case["quorum_safety_margin_bps"]),
        "--min-quorum-safety-margin-bps",
        str(case["min_quorum_safety_margin_bps"]),
        "--evidence-complete",
        bool_value(case["evidence_complete"]),
        "--ci-fast-gate",
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema_version": "kamn.governance.stake-slash-risk.replay-matrix.v1",
            "status": status,

#!/usr/bin/env python3
"""Governance quorum attestation replay contract-lane runner."""
...
def parse_args(argv: list[str]) -> tuple[int, Path]:
    ...
    output_file = Path("/tmp/governance-quorum-attestation-replay-contract-report.json")
    index = 0
...
def main(argv: list[str]) -> int:
    """Execute governance quorum attestation replay contract-lane checks."""
    ...
    if not lane_script.is_file() or not os.access(lane_script, os.X_OK):
        return fail("governance quorum attestation lane script is not executable")
    if not policy_checker.is_file() or not os.access(policy_checker, os.X_OK):
        return fail("governance quorum attestation policy checker script is not executable")

    max_runtime_raw = os.getenv("KAMN_GOVERNANCE_QUORUM_ATTESTATION_CONTRACT_MAX_SECONDS", "240")
    if not max_runtime_raw.isdigit():
        return fail("KAMN_GOVERNANCE_QUORUM_ATTESTATION_CONTRACT_MAX_SECONDS must be an integer >= 0")
    max_runtime = int(max_runtime_raw)
    start_epoch = int(time.time())

    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        go_report = temp_path / "governance-quorum-attestation-go.json"

        go_env = os.environ.copy()
        go_env["KAMN_GOVERNANCE_QUORUM_ATTESTATION_SKIP_COMMANDS"] = "true"
        ...
        if go_code != 0 or "final_decision=GO" not in go_output:
            return fail("expected governance quorum attestation GO path to produce GO decision")
        if "reason_key=governance_quorum_attestation_reason_codes:GO:v1" not in go_output:
            return fail("expected governance quorum attestation GO path reason_key marker")
        ...
        if go_policy_code != 0 or "final_decision=GO" not in go_policy_output:
            return fail("expected governance quorum attestation policy checker GO decision")

        no_go_report = temp_path / "governance-quorum-attestation-no-go.json"
        no_go_env = os.environ.copy()
        no_go_env["KAMN_GOVERNANCE_QUORUM_ATTESTATION_SKIP_COMMANDS"] = "true"
        no_go_env["KAMN_GOVERNANCE_QUORUM_ATTESTATION_FORCE_REPLAY_DETECTED"] = "true"
        ...
        if no_go_code != 0 or "final_decision=NO-GO" not in no_go_output:
            return fail("expected governance quorum attestation replay path to produce NO-GO")
        if "reason_key=governance_quorum_attestation_reason_codes:NO-GO:v1" not in no_go_output:
            return fail("expected governance quorum attestation NO-GO reason_key marker")
        ...
        if no_go_policy_code != 0 or "final_decision=NO-GO" not in no_go_policy_output:
            return fail("expected governance quorum attestation policy checker NO-GO decision")

        tampered_report = temp_path / "governance-quorum-attestation-tampered.json"
        ...
        if tampered_code == 0:
            return fail("expected governance quorum attestation reason_key tamper to fail policy checker")
        if "reason_key mismatch" not in tampered_output:
            return fail(
                "expected explicit reason_key mismatch failure from governance quorum attestation policy checker"
            )
        ...
        if runtime_seconds > max_runtime:
            return fail(
                "governance quorum attestation contract lane exceeded runtime budget "
        ...
        print("governance quorum attestation replay contract lane tests passed.")
        return 0

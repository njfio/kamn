...
if ! grep -q "governance_lifecycle_rollback_contract_lane_contract.py" "$CONTRACT_SCRIPT"; then
  echo "expected governance lifecycle/rollback contract lane wrapper to dispatch to shared module" >&2
  exit 1
fi

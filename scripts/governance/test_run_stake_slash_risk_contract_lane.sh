...
CONTRACT_WRAPPER="run_stake_slash_risk_contract_lane.sh"
...
if ! grep -Fq "governance_stake_slash_risk_contract_lane.json" "$DEEP_LANE"; then
  echo "expected deep lane script to invoke stake/slash contract manifest first" >&2
  exit 1
fi

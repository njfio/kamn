...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture",
        default=str(ROOT_DIR / "fixtures/governance_simulation/veto_timelock_cases.json"),
    )
...
def run_generator(case: Dict[str, Any], bundle_path: Path) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        "--simulation-complete",
        bool_value(case["simulation_complete"]),
        "--veto-window-open",
        bool_value(case["veto_window_open"]),
        "--veto-recorded",
        bool_value(case["veto_recorded"]),
        "--timelock-expired",
        bool_value(case["timelock_expired"]),
        "--required-approvals",
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema_version": "kamn.governance.simulation-veto.replay-matrix.v1",
            "status": status,

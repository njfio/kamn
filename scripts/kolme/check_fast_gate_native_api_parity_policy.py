...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate fast-gate native Kolme API parity lane summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    ci_fast_gate = report.get("ci_fast_gate")
    if ci_fast_gate not in ("PASS", "FAIL"):
        reason_codes.append("ci_fast_gate_invalid")
    elif ci_fast_gate != args.ci_fast_gate:
        reason_codes.append("ci_fast_gate_mismatch")
    ...
    for check in checks:
        ...
        if not isinstance(check_id, str) or not check_id:
            reason_codes.append("check_id_missing")
        else:
            observed_check_ids.add(check_id)
        if check_status not in ("pass", "fail"):
            reason_codes.append("check_status_invalid")

    if observed_check_ids and observed_check_ids != expected_check_ids:
        reason_codes.append("check_id_set_mismatch")

    observed_final_decision = ""
    if status == "ok":
        observed_final_decision = "GO"
        if reason_code != "fast_gate_native_api_parity_passed":
            reason_codes.append("ok_reason_code_mismatch")
        if budget_status != "within_budget":
            reason_codes.append("ok_budget_status_mismatch")
    elif status == "fail":
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.fast-gate-native-api-parity-policy-report.v1",

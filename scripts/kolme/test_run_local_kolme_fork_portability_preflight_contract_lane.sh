...
RUN_WRAPPER_IMPL="$ROOT_DIR/scripts/kolme/run_local_kolme_fork_portability_preflight_lane_impl.sh"
RUN_MANIFEST="$ROOT_DIR/scripts/framework/manifests/kolme_local_kolme_fork_portability_preflight_lane.json"
DISPATCHER="$ROOT_DIR/scripts/kolme/run_lane_dispatch.sh"
CONTRACT_IMPL="$ROOT_DIR/scripts/kolme/contracts/local_fork_portability_preflight_contract_lane.py"
...
if [ ! -x "$RUN_WRAPPER_IMPL" ]; then
  echo "expected local fork portability preflight implementation runner to be executable" >&2
  exit 1
fi
...
if [ ! -L "$RUN_WRAPPER" ]; then
  echo "expected local fork portability preflight runner to be a symlink to shared runtime lane dispatcher" >&2
  exit 1
fi

if [ "$(readlink "$RUN_WRAPPER")" != "run_lane_dispatch.sh" ]; then
  echo "expected local fork portability preflight runner symlink target to be run_lane_dispatch.sh" >&2
  exit 1
fi

if [ ! -f "$RUN_MANIFEST" ]; then
  echo "expected local fork portability preflight run manifest to exist" >&2
  exit 1
fi
...
if payload.get("schema_version") != "kamn.contract-lane.manifest.v1":
    raise SystemExit("expected local fork portability preflight run manifest schema")
if payload.get("lane_id") != "kolme.local_kolme_fork_portability_preflight.run":
    raise SystemExit("expected local fork portability preflight run manifest lane_id")
run_command = payload.get("phases", {}).get("run")
if run_command != [
    "bash",
    "scripts/kolme/run_local_kolme_fork_portability_preflight_lane_impl.sh",
]:
    raise SystemExit("expected local fork portability preflight run manifest command")
PY
...
if [ "$resolved_run_manifest_path" != "$RUN_MANIFEST" ]; then
  echo "expected local fork portability preflight wrapper to resolve deterministic run manifest" >&2
  exit 1
fi

if bash "$DISPATCHER" --lane-wrapper run_missing_local_kolme_fork_portability_preflight_lane.sh --resolve-manifest-path >/dev/null 2>&1; then
  echo "expected local run lane dispatcher to fail closed for unknown local fork portability preflight wrapper" >&2
  exit 1
fi
...
if contract_command != [
    ...
    raise SystemExit("expected local fork portability preflight manifest contract command")
PY

if [ ! -f "$CONTRACT_IMPL" ]; then
  echo "expected local fork portability preflight contract implementation to exist" >&2
  exit 1
fi
...
for marker in "${required_coverage_markers[@]}"; do
  if ! grep -q "$marker" "$CONTRACT_IMPL"; then
    echo "expected local fork portability preflight contract implementation to include coverage marker: $marker" >&2
    exit 1
  fi
done
...
# Regression: #1707
if ! grep -q "Regression: #1707" "$DOC_FILE"; then
  echo "expected Kolme devnet ops doc to include portability preflight contract-lane regression marker" >&2
  exit 1
fi
...
if summary.get("schema_version") != "kamn.kolme.local-fork-portability-preflight-summary.v1":
    raise SystemExit("unexpected portability preflight contract-lane summary schema")
if summary.get("status") != "ok":
    raise SystemExit("expected portability preflight contract-lane summary status ok")
if summary.get("reason_code") != "dry_run_no_commands_executed":
    raise SystemExit("expected dry-run reason code in portability preflight contract-lane summary")
if policy.get("schema_version") != "kamn.kolme.local-fork-portability-preflight-policy-report.v1":
    raise SystemExit("unexpected portability preflight contract-lane policy schema")
if policy.get("final_decision") != "GO":
    raise SystemExit("expected portability preflight contract-lane policy final_decision GO")
PY

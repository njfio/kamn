...
python3 "$ROOT_DIR/scripts/kolme/contracts/local_kolme_fork_rust_test_matrix_summary.py" "$OUTPUT_JSON" "$MODE" "$overall_status" "$reason_code" "$local_only_enforced" "$elapsed_seconds" "$MAX_SECONDS" "$budget_status" "$CHECKOUT_PATH" "$EXPECTED_REMOTE_URL" "$EXPECTED_REF" "$METADATA_REPORT" "$COMMAND_OUTPUT_DIR" "$CHECK_FILE" "$CARGO_PROFILE" "${MATRIX_COMMANDS[@]}"

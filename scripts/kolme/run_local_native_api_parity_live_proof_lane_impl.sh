...
python3 - "$OUTPUT_JSON" "$MODE" "$overall_status" "$reason_code" "$local_only_enforced" "$elapsed_seconds" "$MAX_SECONDS" "$budget_status" "$planned_nonce_command" "$planned_broadcast_command" "$planned_finality_command" "$nonce_log" "$broadcast_log" "$finality_log" "$CHECK_FILE" <<'PY'

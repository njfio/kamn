...
python3 - "$OUTPUT_JSON" "$MODE" "$overall_status" "$reason_code" "$CHECKOUT_PATH" "$EXPECTED_REMOTE_URL" "$EXPECTED_REF" "$BASE_URL" "$FORK_CHAIN_VERSION" "$elapsed_seconds" "$MAX_SECONDS" "$budget_status" "$SYNC_REPORT" "$PROBE_REPORT" "$sync_reason_code" "$probe_reason_code" "$CHECK_FILE" <<'PY'
from __future__ import annotations

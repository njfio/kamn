...
EXPECTED_COMMAND_SNIPPETS = {
    "bootstrap_health_checks": "run_local_bootstrap_health_checks.sh",
    "fork_checkout_bootstrap_contract": "run_local_kolme_fork_checkout_bootstrap_contract_lane.sh",
    "runtime_commit_adapter": "run_runtime_commit_adapter_contract_lane.sh",
    "sdk_live_transport_parity": "run_live_transport_parity_contract_lane.sh",
    "fork_rust_test_matrix": "run_local_kolme_fork_rust_test_matrix_contract_lane.sh",
    "fork_live_api_conformance": "run_local_kolme_live_api_conformance_contract_lane.sh",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local E2E integration summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    if status == "ok":
        ...
        if mode == "run" and reason_code != "local_e2e_integration_passed":
        ...
        if reason_code in ("dry_run_no_commands_executed", "local_e2e_integration_passed"):
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.local-e2e-integration-policy-report.v1",

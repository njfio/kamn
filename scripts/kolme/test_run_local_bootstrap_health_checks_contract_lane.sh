...
LEGACY_RUNNER="$ROOT_DIR/scripts/kolme/run_local_bootstrap_health_checks_contract_lane.sh"
...
MANIFEST="$ROOT_DIR/scripts/framework/manifests/kolme_local_bootstrap_health_checks_contract_lane.json"
CONTRACT_IMPL="$ROOT_DIR/scripts/kolme/contracts/local_bootstrap_health_checks_contract_lane.py"
...
if [ -e "$LEGACY_RUNNER" ]; then
  echo "expected superseded local bootstrap health-check contract lane wrapper to be deleted" >&2
  exit 1
fi
...
if [ ! -f "$MANIFEST" ]; then
  echo "expected local bootstrap health-check contract lane manifest to exist" >&2
  exit 1
fi
...
if contract_command != [
    ...
    raise SystemExit("expected local bootstrap manifest contract command")
PY

if [ ! -f "$CONTRACT_IMPL" ]; then
  echo "expected local bootstrap health-check contract implementation to exist" >&2
  exit 1
fi
...
for marker in "${required_coverage_markers[@]}"; do
  if ! grep -q "$marker" "$CONTRACT_IMPL"; then
    echo "expected local bootstrap health-check contract implementation to include coverage marker: $marker" >&2
    exit 1
  fi
done

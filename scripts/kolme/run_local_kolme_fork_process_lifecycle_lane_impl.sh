...
python3 "$ROOT_DIR/scripts/kolme/contracts/local_kolme_fork_process_lifecycle_summary.py" "$OUTPUT_JSON" "$MODE" "$overall_status" "$reason_code" "$CHECKOUT_PATH" "$EXPECTED_REMOTE_URL" "$EXPECTED_REF" "$BASE_URL" "$FORK_CHAIN_VERSION" "$elapsed_seconds" "$MAX_SECONDS" "$budget_status" "$SERVE_COMMAND" "$PROCESS_OUTPUT_FILE" "$INTEGRATION_REPORT" "$INTEGRATION_RUNTIME_COMMIT_LIVE_POLICY_REPORT" "$integration_runtime_commit_policy_reason_code" "$INTEGRATION_RUNTIME_COMMIT_FINALITY_COMMAND" "$INTEGRATION_RUNTIME_COMMIT_FINALITY_MAX_SECONDS" "$INTEGRATION_RUNTIME_COMMIT_FINALITY_OUTPUT_FILE" "$ROLLBACK_EVIDENCE_FILE" "$RECOVERY_EVIDENCE_FILE" "$rollback_evidence_status" "$rollback_evidence_reason_code" "$recovery_evidence_status" "$recovery_evidence_reason_code" "$start_reason_code" "$readiness_reason_code" "$integration_reason_code" "$teardown_reason_code" "$CHECK_FILE"

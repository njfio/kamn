...
if ! grep -q "scripts/framework/assert_local_heavy_opt_in.sh" "$RUNNER"; then
  echo "expected local KAMN live runtime integration runner to invoke shared local-heavy opt-in guard helper" >&2
  exit 1
fi
...
if [ ! -x "$CONTRACT_LANE" ]; then
  echo "expected local KAMN live runtime integration contract lane to be executable" >&2
  exit 1
fi

if ! grep -q "run_local_kamn_live_runtime_integration_lane.sh" "$DOC_FILE"; then
  echo "expected Kolme devnet ops doc to reference local KAMN live runtime integration runner" >&2
  exit 1
fi
...
git -C "$CHECKOUT_PATH" commit -q -m "init runtime integration fixture"
...
assert_eq "$(extract_value "$dry_run_output" "status")" "ok" "expected dry-run runtime integration lane to pass"
...
if ! grep -q "requires explicit local-only opt-in" "$TMP_ERR"; then
  echo "expected deterministic local-only opt-in failure message for runtime integration lane" >&2
  exit 1
fi
...
assert_eq "$(extract_value "$run_output" "status")" "ok" "expected run mode runtime integration lane to pass"
assert_eq "$(extract_value "$run_output" "lane_mode")" "run" "expected run mode marker"
assert_eq "$(extract_value "$run_output" "reason_code")" "live_runtime_integration_passed" "expected run-mode success reason code"
...
checker_run_output="$(
  python3 "$CHECKER" \
    ...
    --expected-final-decision GO \
    --ci-fast-gate PASS \
    --require-reason-code live_runtime_integration_passed \
    --output-json "$TMP_POLICY"
)"
...
if report.get("schema_version") != "kamn.kolme.local-kamn-live-runtime-integration-summary.v1":
    raise SystemExit("unexpected runtime integration summary schema")
...
if not isinstance(contracts, dict):
    raise SystemExit("expected contracts map in summary")
...
if ! printf '%s\n' "$contract_output" | grep -q "local KAMN live runtime integration contract lane tests passed."; then
  echo "expected local KAMN live runtime integration contract lane success marker" >&2
  exit 1
fi

...
POLICY_OUTPUT_JSON="/tmp/kolme-local-kamn-live-runtime-integration-policy.json"
MAX_SECONDS="${KAMN_KOLME_LOCAL_KAMN_RUNTIME_INTEGRATION_MAX_SECONDS:-240}"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'USAGE'
Usage: run_local_kamn_live_runtime_integration_contract_lane.sh [options]

Options:
  --output-json <path>         Runtime integration summary output.
...
if [ "$elapsed_seconds" -gt "$MAX_SECONDS" ]; then
  echo "local KAMN live runtime integration contract lane exceeded runtime budget: ${elapsed_seconds}s" >&2
  exit 1
fi

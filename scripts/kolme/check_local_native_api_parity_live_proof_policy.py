...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local native API parity live-proof lane summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    if status == "ok":
        ...
        if reason_code not in ("dry_run_no_commands_executed", "native_parity_live_proof_passed"):
        ...
        if reason_code in ("dry_run_no_commands_executed", "native_parity_live_proof_passed"):
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.local-native-api-parity-live-proof-policy-report.v1",

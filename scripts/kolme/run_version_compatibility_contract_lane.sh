...
if [ ! -x "$BLOCK_FALLBACK_LANE" ]; then
  echo "expected Kolme block fallback reconciliation contract lane script to be executable" >&2
  exit 1
fi
...
bash "$BLOCK_FALLBACK_LANE" >/dev/null
...
if ! grep -q "run_block_fallback_reconciliation_contract_lane.sh" "$ROADMAP_DOC"; then
  echo "expected Kolme roadmap doc to reference block fallback reconciliation contract lane command" >&2
  exit 1
fi

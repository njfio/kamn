...
POLICY_OUTPUT_JSON="/tmp/kolme-local-fork-profile-preflight-policy.json"
MAX_SECONDS="${KAMN_KOLME_LOCAL_FORK_PROFILE_PREFLIGHT_CONTRACT_MAX_SECONDS:-45}"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'USAGE'
Usage: run_local_kolme_fork_profile_preflight_contract_lane.sh [options]

Options:
  --output-json <path>         Preflight summary output.
  --policy-output-json <path>  Preflight policy report output.

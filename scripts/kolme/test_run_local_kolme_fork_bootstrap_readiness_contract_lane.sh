...
RUN_WRAPPER_IMPL="$ROOT_DIR/scripts/kolme/run_local_kolme_fork_bootstrap_readiness_lane_impl.sh"
RUN_MANIFEST="$ROOT_DIR/scripts/framework/manifests/kolme_local_kolme_fork_bootstrap_readiness_lane.json"
...
if [ ! -x "$RUN_WRAPPER_IMPL" ]; then
  echo "expected local fork bootstrap/readiness implementation runner to be executable" >&2
  exit 1
fi
...
if [ ! -L "$RUN_WRAPPER" ]; then
  echo "expected local fork bootstrap/readiness runner to be a symlink to shared runtime lane dispatcher" >&2
  exit 1
fi

if [ "$(readlink "$RUN_WRAPPER")" != "run_lane_dispatch.sh" ]; then
  echo "expected local fork bootstrap/readiness runner symlink target to be run_lane_dispatch.sh" >&2
  exit 1
fi

if [ ! -f "$RUN_MANIFEST" ]; then
  echo "expected local fork bootstrap/readiness run manifest to exist" >&2
  exit 1
fi
...
if payload.get("schema_version") != "kamn.contract-lane.manifest.v1":
    raise SystemExit("expected local fork bootstrap/readiness run manifest schema")
if payload.get("lane_id") != "kolme.local_kolme_fork_bootstrap_readiness.run":
    raise SystemExit("expected local fork bootstrap/readiness run manifest lane_id")
run_command = payload.get("phases", {}).get("run")
if run_command != [
    "bash",
    "scripts/kolme/run_local_kolme_fork_bootstrap_readiness_lane_impl.sh",
]:
    raise SystemExit("expected local fork bootstrap/readiness run manifest command")
PY
...
if [ "$resolved_run_manifest_path" != "$RUN_MANIFEST" ]; then
  echo "expected local fork bootstrap/readiness wrapper to resolve deterministic run manifest" >&2
  exit 1
fi

if bash "$DISPATCHER" --lane-wrapper run_missing_local_kolme_fork_bootstrap_readiness_lane.sh --resolve-manifest-path >/dev/null 2>&1; then
  echo "expected local run lane dispatcher to fail closed for unknown local fork bootstrap/readiness wrapper" >&2
  exit 1
fi

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local Kolme fork bootstrap/readiness summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    if not isinstance(checks, list) or not checks:
        ...
        expected_ids = {"fork_metadata_sync", "api_probe"}
    ...
    if status == "ok":
        ...
        if reason_code not in ("dry_run_no_commands_executed", "bootstrap_readiness_passed"):
        ...
        if reason_code in ("dry_run_no_commands_executed", "bootstrap_readiness_passed"):
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.local-fork-bootstrap-readiness-policy-report.v1",

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local Kolme fork self-test summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    matrix_max_seconds = report.get("matrix_max_seconds")
    ...
    if not isinstance(contracts, dict):
        ...
        if contracts.get("matrix_runner") != "run_local_kolme_fork_rust_test_matrix_lane.sh":
            reason_codes.append("matrix_runner_mismatch")
        if contracts.get("matrix_checker") != "check_local_kolme_fork_rust_test_matrix_policy.py":
            reason_codes.append("matrix_checker_mismatch")
        if contracts.get("matrix_schema") != "kamn.kolme.local-fork-rust-test-matrix-summary.v1":
            reason_codes.append("matrix_schema_mismatch")
    ...
    if status == "ok":
        ...
        if reason_code not in ("dry_run_no_commands_executed", "fork_self_test_passed"):
        ...
        if reason_code in ("dry_run_no_commands_executed", "fork_self_test_passed"):
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.local-fork-self-test-policy-report.v1",

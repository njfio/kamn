...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local Kolme live API conformance harness summary policy."
    )
...
def evaluate(report: dict[str, object], args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    if not isinstance(checks, list) or not checks:
        ...
        expected_ids = {"api_probe", "native_api_parity"}
    ...
    if not isinstance(contracts, dict):
        ...
        if contracts.get("fork_info_query_key") != "chain_version":
            reason_codes.append("fork_info_query_key_mismatch")
        if contracts.get("nonce_endpoint") != "/get-next-nonce":
            reason_codes.append("nonce_endpoint_mismatch")
        if contracts.get("broadcast_endpoint") != "/broadcast":
            reason_codes.append("broadcast_endpoint_mismatch")
        if contracts.get("broadcast_method") != "PUT":
            reason_codes.append("broadcast_method_mismatch")
        broadcast_payload = contracts.get("broadcast_payload")
        if not isinstance(broadcast_payload, str) or not broadcast_payload.strip():
            reason_codes.append("broadcast_payload_missing")
    ...
    if status == "ok":
        ...
        if reason_code not in ("dry_run_no_commands_executed", "live_api_conformance_passed"):
        ...
        if reason_code in ("dry_run_no_commands_executed", "live_api_conformance_passed"):
...
def main() -> int:
    ...
    output = {
        "schema_version": "kamn.kolme.local-live-api-conformance-policy-report.v1",

...
if payload.get("subscription_taxonomy_status") != "verified":
    raise SystemExit("expected subscription_taxonomy_status=verified")
PY
...
if ! grep -Fq "subscription_reason_codes_csv=${SUBSCRIPTION_REASON_CODES_CSV}" "$RUST_SDK_DOC"; then
  echo "expected rust sdk docs to include subscription reason-codes marker" >&2
  exit 1
fi
...
if ! grep -Fq "subscription_reason_taxonomy_version=${SUBSCRIPTION_REASON_TAXONOMY_VERSION}" "$SDK_README_DOC"; then
  echo "expected sdk README to include subscription taxonomy version marker" >&2
  exit 1
fi

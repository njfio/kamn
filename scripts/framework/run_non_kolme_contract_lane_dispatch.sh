...
resolve_manifest_record() {
  ...
  python3 "$KAMN_ROOT/scripts/framework/resolve_non_kolme_manifest.py" \
...
if [[ "$RESOLVE_STATUS" != "ok" ]]; then
  ...
  case "$RESOLVE_ERROR_CODE" in
    unknown_wrapper)
      ...
      ;;
    invalid_phase|duplicate_wrapper|invalid_manifest)
      emit_fallback_error \
...
if [[ -z "$PHASE_NAME" ]]; then
  emit_fallback_error \
    "dispatcher_phase_unmapped" \
    "manifest resolver returned empty phase for wrapper: $WRAPPER_NAME"
  exit 1
fi

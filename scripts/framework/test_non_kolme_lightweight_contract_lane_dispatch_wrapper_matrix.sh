...
if bash "$DISPATCHER" --lane-wrapper run_missing_non_kolme_lightweight_contract_lane.sh --resolve-manifest-path >/dev/null 2>&1; then
  echo "expected non-Kolme dispatcher to fail for unknown lightweight wrapper" >&2
  exit 1
fi

echo "non-Kolme lightweight contract lane dispatcher wrapper matrix tests passed."

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture",
        default=str(ROOT_DIR / "fixtures/reputation_dispute/replay_cases.json"),
    )
...
def run_generator(case: Dict[str, Any], bundle_path: Path) -> subprocess.CompletedProcess[str]:
    command = [
        ...
        "--reviewer-did",
        str(case["reviewer_did"]),
        "--dispute-reason-code",
        str(case["dispute_reason_code"]),
        "--evidence-uri",
        str(case["evidence_uri"]),
        "--evidence-sha256",
        str(case["evidence_sha256"]),
        "--evidence-hash-verified",
        str(case["evidence_hash_verified"]),
        "--original-trust-score",
        str(case["original_trust_score"]),
        "--proposed-trust-score",
        str(case["proposed_trust_score"]),
        "--max-adjustment-points",
        str(case["max_adjustment_points"]),
        "--policy-window-open",
        bool_value(case["policy_window_open"]),
        "--approval-recorded",
        bool_value(case["approval_recorded"]),
        "--ci-fast-gate",
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema_version": "kamn.reputation.dispute.replay-matrix.v1",
            "status": status,

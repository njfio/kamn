#!/usr/bin/env python3
"""Service API shutdown abrupt-close regression lane and policy contracts."""
...
RUN_LANE_SCHEMA = "kamn.runtime.service-api-shutdown-abrupt-close-regression-live-report.v1"
POLICY_SCHEMA = "kamn.runtime.service-api-shutdown-abrupt-close-regression-live-policy-report.v1"
OPT_IN_ENV = "KAMN_LOCAL_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_OPT_IN"

ABRUPT_CLOSE_REGRESSION_TESTS: list[tuple[str, str]] = [
    (
        "abrupt_close_guard",
        "main_tests::service_api_endpoint_tests::regression_service_api_endpoint_websocket_route_rejects_missing_upgrade_headers",
    ),
    (
        "websocket_rejection_guard",
        "main_tests::service_api_endpoint_tests::regression_service_api_endpoint_websocket_rejects_invalid_version_header",
    ),
    (
    ...
    (
        "replayed_signal_guard",
        "main_tests::cli_contract_tests::regression_runtime_daemon_ignores_replayed_and_late_shutdown_signals",
    ),
]
...
def _run_cargo_test(selector: str, *, timeout_seconds: int) -> str:
    ...
    try:
        ...
        fail(
            "shutdown abrupt-close regression command timed out: "
            f"{selector} (timeout={timeout_seconds}s): {error}"
        )

    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout or "command failed").strip()
        fail(f"shutdown abrupt-close regression command failed for {selector}: {detail}")
...
def _run_lane(args: argparse.Namespace) -> int:
    ...
    max_seconds = require_positive_int(
        "KAMN_SERVICE_API_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_MAX_SECONDS",
        args.max_seconds,
    )
    command_max_seconds = require_positive_int(
        "KAMN_SERVICE_API_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_COMMAND_MAX_SECONDS",
    ...
    if mode == "run":
        ...
        for _, selector in ABRUPT_CLOSE_REGRESSION_TESTS:
            commands.append(
    ...
    if elapsed_seconds > max_seconds:
        fail(
            "service api shutdown abrupt-close regression lane exceeded runtime budget: "
    ...
    print("abrupt_close_guard_status=verified")
    print("websocket_rejection_guard_status=verified")
    print("shutdown_timeout_guard_status=verified")
    print("replayed_signal_guard_status=verified")
...
def _check_policy(args: argparse.Namespace) -> int:
    ...
    required_fields = [
        ...
        "final_decision",
        "lane_mode",
        "abrupt_close_guard_status",
        "websocket_rejection_guard_status",
        "shutdown_timeout_guard_status",
        "replayed_signal_guard_status",
        "fail_closed_status",
    ...
    decision.reject_if(
        ...
        "service_api_shutdown_abrupt_close_regression_policy_schema_mismatch",
    )
    decision.reject_if(
        report.get("status") not in {"pass", "fail"},
        "service_api_shutdown_abrupt_close_regression_policy_status_invalid",
    )
    decision.reject_if(
        report.get("final_decision") not in {"GO", "NO-GO"},
        "service_api_shutdown_abrupt_close_regression_policy_final_decision_invalid",
    )
    decision.reject_if(
        report.get("final_decision") != expected_final_decision,
        "service_api_shutdown_abrupt_close_regression_policy_final_decision_mismatch",
    )
    ...
    for field_name in (
        ...
        decision.reject_if(
            ...
            f"service_api_shutdown_abrupt_close_regression_policy_marker_missing:{field_name}",
        )

    lane_mode = report.get("lane_mode")
    decision.reject_if(
        lane_mode not in {"dry-run", "run"},
        "service_api_shutdown_abrupt_close_regression_policy_lane_mode_invalid",
    )

    command_count = report.get("command_count")
    decision.reject_if(
        not _is_non_negative_int(command_count),
        "service_api_shutdown_abrupt_close_regression_policy_command_count_invalid",
    )
    ...
    if lane_mode == "dry-run":
        decision.reject_if(
            execution_reason_code != "dry_run_no_commands_executed",
            "service_api_shutdown_abrupt_close_regression_policy_execution_reason_code_mismatch",
        )
        decision.reject_if(
            command_count != 0,
            "service_api_shutdown_abrupt_close_regression_policy_command_count_mismatch",
        )
    elif lane_mode == "run":
        decision.reject_if(
            execution_reason_code != "run_mode_commands_executed",
        ...
        decision.reject_if(
            ...
            or command_count < len(ABRUPT_CLOSE_REGRESSION_TESTS),
            "service_api_shutdown_abrupt_close_regression_policy_command_count_mismatch",
        )

    decision.reject_if(
        not _is_non_negative_int(report.get("elapsed_seconds")),
        "service_api_shutdown_abrupt_close_regression_policy_elapsed_seconds_invalid",
    )
    ...
    policy_report = {
        ...
        "service_api_shutdown_abrupt_close_regression_policy_status": policy_status,
    ...
    print(f"service_api_shutdown_abrupt_close_regression_policy_status={policy_status}")
    ...
    if final_decision != "GO":
        fail(
            "service api shutdown abrupt-close regression live policy rejected: "
            f"{reason_codes_csv}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Service API shutdown abrupt-close regression lane and policy contracts."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_lane_parser = subparsers.add_parser(
        "run-lane",
        help="Execute service API shutdown abrupt-close regression lane in dry-run or run mode.",
    )
    run_lane_parser.add_argument(
        "--mode",
        default=os.environ.get(
            "KAMN_SERVICE_API_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_MODE", "dry-run"
        ),
    ...
    run_lane_parser.add_argument(
        "--max-seconds",
        default=os.environ.get(
            "KAMN_SERVICE_API_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_MAX_SECONDS", "180"
        ),
    ...
    run_lane_parser.add_argument(
        "--command-max-seconds",
        default=os.environ.get(
            "KAMN_SERVICE_API_SHUTDOWN_ABRUPT_CLOSE_REGRESSION_COMMAND_MAX_SECONDS", "120"
        ),
    ...
    check_policy_parser = subparsers.add_parser(
        "check-policy",
        help="Validate service API shutdown abrupt-close regression report policy.",
    )
    check_policy_parser.add_argument(
        "--report-file",
        required=True,
        help="Path to service API shutdown abrupt-close regression report JSON.",
    )
    check_policy_parser.add_argument(
        "--expected-final-decision",
        default="GO",

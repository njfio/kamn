...
report_file="$TMP_DIR/service-api-prometheus-metrics-live-summary.json"
bash "$ROOT_DIR/scripts/lib/write_json_file.sh" "$report_file" <<'JSON'
{
  "schema_version": "kamn.runtime.service-api-prometheus-metrics-live-report.v1",
  "status": "pass",
  "final_decision": "GO",
  "lane_mode": "dry-run",
...
if ! printf '%s\n' "$policy_output" | grep -q '^final_decision=GO$'; then
  echo "expected service api prometheus metrics policy checker GO marker" >&2
  exit 1
fi
...
if payload.get("schema_version") != "kamn.runtime.service-api-prometheus-metrics-live-policy-report.v1":
...
if payload.get("service_api_prometheus_metrics_policy_status") != "verified":
...
if [ "$tampered_code" -eq 0 ]; then
  echo "expected tampered service api prometheus metrics report to fail policy checker" >&2
  exit 1
fi
if ! printf '%s\n' "$tampered_output" | grep -q 'service_api_prometheus_metrics_policy_marker_missing:metrics_contract_status'; then
  echo "expected deterministic mismatch reason code for tampered prometheus metrics policy validation" >&2
  exit 1
fi

echo "service api prometheus metrics live policy checker tests passed."

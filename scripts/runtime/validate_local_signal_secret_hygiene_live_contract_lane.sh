...
output_json=""
policy_output_json=""
max_seconds="${KAMN_LOCAL_SIGNAL_SECRET_HYGIENE_CONTRACT_MAX_SECONDS:-240}"
ci_fast_gate="PASS"
mode="dry-run"
...
summary_report="$TMP_DIR/runtime-local-signal-secret-hygiene-summary.json"
policy_report="$TMP_DIR/runtime-local-signal-secret-hygiene-policy.json"
tampered_report="$TMP_DIR/runtime-local-signal-secret-hygiene-summary.tampered.json"
...
if ! printf '%s\n' "$validation_output" | grep -q '^status=pass$'; then
  echo "expected local signal/secret hygiene live validation status=pass marker" >&2
  exit 1
fi
if ! printf '%s\n' "$validation_output" | grep -q '^final_decision=GO$'; then
  echo "expected local signal/secret hygiene live validation final_decision=GO marker" >&2
  exit 1
fi
...
payload["fallback_secret_fail_closed_reason_code"] = "tampered-reason-code"
...
tampered_policy_output="$(
  bash "$POLICY_CHECKER" \
    ...
    --expected-final-decision GO \
    --ci-fast-gate "$ci_fast_gate" \
    --output-json "$TMP_DIR/runtime-local-signal-secret-hygiene-policy.tampered.json" 2>&1
)"
...
if ! printf '%s\n' "$tampered_policy_output" | grep -q 'local_signal_secret_hygiene_policy_secret_reason_code_mismatch'; then
  echo "expected deterministic fail-closed reason for tampered local signal/secret hygiene report" >&2
  exit 1
fi

for required_ref in \
  "validate_local_signal_secret_hygiene_live.sh" \
  "check_local_signal_secret_hygiene_live_policy.sh" \
  "validate_local_signal_secret_hygiene_live_contract_lane.sh" \
  "test_validate_local_signal_secret_hygiene_live.sh" \
  "test_check_local_signal_secret_hygiene_live_policy.sh" \
  "test_validate_local_signal_secret_hygiene_live_contract_lane.sh"; do
  if ! grep -q "$required_ref" "$STRATEGY_DOC"; then
    echo "expected CI strategy docs to reference $required_ref" >&2
    exit 1
  fi
done
if ! grep -q "local signal/secret hygiene run-mode commands remain excluded from ci-fast-gate and ci-tools fast mode." "$STRATEGY_DOC"; then
  echo "expected CI strategy docs to include local signal/secret hygiene run-mode exclusion marker" >&2
  exit 1
fi

if ! grep -q "Task #3219, Subtask #3220" "$ROADMAP_DOC"; then
  echo "expected roadmap marker for Task #3219, Subtask #3220" >&2
  exit 1
fi
if ! grep -q "scripts/runtime/validate_local_signal_secret_hygiene_live_contract_lane.sh" "$ROADMAP_DOC"; then
  echo "expected roadmap to reference local signal/secret hygiene contract lane script" >&2
  exit 1
fi
if ! grep -q "scripts/runtime/check_local_signal_secret_hygiene_live_policy.sh" "$ROADMAP_DOC"; then
  echo "expected roadmap to reference local signal/secret hygiene policy checker script" >&2
  exit 1
fi

elapsed_seconds="$(( $(date +%s) - start_epoch ))"
if [ "$elapsed_seconds" -gt "$max_seconds" ]; then
  echo "local signal/secret hygiene contract lane exceeded runtime budget: ${elapsed_seconds}s" >&2
  exit 1
fi
...
if summary_report.get("schema_version") != "kamn.runtime.local-signal-secret-hygiene-live-report.v1":
    raise SystemExit("unexpected local signal/secret hygiene live summary schema")
if policy_report.get("schema_version") != "kamn.runtime.local-signal-secret-hygiene-live-policy-report.v1":
    raise SystemExit("unexpected local signal/secret hygiene policy schema")
if summary_report.get("final_decision") != "GO":
    raise SystemExit("expected local signal/secret hygiene summary final_decision=GO")
if policy_report.get("final_decision") != "GO":
    raise SystemExit("expected local signal/secret hygiene policy final_decision=GO")

lane_report = {
    "schema_version": "kamn.runtime.local-signal-secret-hygiene-live-contract-lane-report.v1",
    "status": "pass",
    "final_decision": "GO",
    "lane_mode": mode,
    "local_signal_secret_hygiene_contract_status": "verified",

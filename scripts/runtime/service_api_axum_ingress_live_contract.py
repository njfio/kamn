...
CONVERGENCE_SCHEMA = "kamn.runtime.service-api-axum-ingress-live-convergence-report.v1"
...
EXPECTED_EVIDENCE_CONVERGENCE_REASON_TAXONOMY_VERSION = (
    "kamn.runtime.service-api-axum-evidence-convergence-reason-taxonomy.v1"
)
EXPECTED_EVIDENCE_CONVERGENCE_REASON_CODES_CSV = (
    "service_api_axum_evidence_link_missing,"
    "service_api_axum_evidence_payload_tamper_detected,"
...
def _check_evidence_convergence(args: argparse.Namespace) -> int:
    ...
    decision.reject_if(
        ...
        "service_api_axum_evidence_payload_tamper_detected:report_schema_version",
    )
    decision.reject_if(
        policy.get("schema_version") != POLICY_SCHEMA,
        "service_api_axum_evidence_payload_tamper_detected:policy_schema_version",
    )
    ...
    decision.reject_if(
        ...
        "service_api_axum_evidence_payload_tamper_detected:final_decision",
    )
    decision.reject_if(
        policy_final_decision not in {"GO", "NO-GO"},
        "service_api_axum_evidence_payload_tamper_detected:policy_final_decision",
    )
    decision.reject_if(
        (
    ...
    decision.reject_if(
        report.get("service_api_axum_ingress_policy_status")
        != policy.get("service_api_axum_ingress_policy_status"),
        "service_api_axum_evidence_payload_tamper_detected:service_api_axum_ingress_policy_status",
    )
    ...
    if not isinstance(source_report_file, str) or source_report_file.strip() == "":
        ...
        if source_report_path.is_file():
            try:
                ...
                decision.reject_if(
                    True,
                    "service_api_axum_evidence_payload_tamper_detected:source_report_file",
                )

    if source_report is not None:
        decision.reject_if(
            source_report.get("schema_version") != REPORT_SCHEMA,
            "service_api_axum_evidence_payload_tamper_detected:source_report_schema_version",
        )
        source_report_final_decision = source_report.get("final_decision")
        decision.reject_if(
            source_report_final_decision not in {"GO", "NO-GO"},
            "service_api_axum_evidence_payload_tamper_detected:source_report_final_decision",
        )
        decision.reject_if(
            (
    ...
    if _is_non_empty_string_list(policy_reason_codes):
        ...
        decision.reject_if(
            True,
            "service_api_axum_evidence_payload_tamper_detected:reason_codes",
        )

    observed_reason_codes_value = policy.get("reason_codes_value")
    decision.reject_if(
        not isinstance(observed_reason_codes_value, str),
        "service_api_axum_evidence_payload_tamper_detected:reason_codes_value",
    )
    ...
    expected_reason_code = _resolve_protocol_mismatch_reason_code(
    ...
    decision.reject_if(
        policy.get("service_api_axum_protocol_mismatch_reason_mapping_status") != "verified",
        "service_api_axum_promotion_decision_reason_mapping_mismatch",
    )
    decision.reject_if(
        policy.get("service_api_axum_protocol_mismatch_reason_taxonomy_version")
        != EXPECTED_PROTOCOL_MISMATCH_REASON_TAXONOMY_VERSION,
        "service_api_axum_promotion_decision_reason_mapping_mismatch",
    )
    decision.reject_if(
        policy.get("service_api_axum_protocol_mismatch_reason_codes_csv")
        != EXPECTED_PROTOCOL_MISMATCH_REASON_CODES_CSV,
        "service_api_axum_promotion_decision_reason_mapping_mismatch",
    )
    decision.reject_if(
        policy.get("service_api_axum_protocol_mismatch_reason_code") != expected_reason_code,
        "service_api_axum_promotion_decision_reason_mapping_mismatch",
    )
    ...
    promotion_decision_reason_mapping_status = (
        "failed"
        if "service_api_axum_promotion_decision_reason_mapping_mismatch" in reason_codes
        else "verified"
    )
    reason_codes_value = ",".join(reason_codes)
    ...
    if final_decision != "GO":
        fail(f"service api axum evidence convergence rejected: {reason_codes_value}")
...
def main() -> int:
    ...
    check_evidence_parser = subparsers.add_parser(
        ...
        help="Validate axum evidence convergence across lane and policy artifacts.",
    )
    check_evidence_parser.add_argument(
        "--report-file",
        required=True,
        help="Path to axum contract-lane report JSON.",
    )
    check_evidence_parser.add_argument(
        "--policy-file",
        required=True,
        help="Path to axum policy report JSON.",
    )
    check_evidence_parser.add_argument(
        "--output-json",

...
if [ ! -L "$CONTRACT_LANE" ]; then
  echo "expected service api shutdown abrupt-close regression contract lane to be a dispatcher symlink" >&2
  exit 1
fi
if [ "$(readlink "$CONTRACT_LANE")" != "run_service_api_tranche2_contract_lane_dispatch.sh" ]; then
  echo "expected service api shutdown abrupt-close regression contract lane symlink target to be tranche-2 dispatcher" >&2
  exit 1
fi
resolved_impl_path="$(bash "$DISPATCHER" --lane-wrapper "$(basename "$CONTRACT_LANE")" --resolve-impl-path)"
if [ "$resolved_impl_path" != "$ROOT_DIR/scripts/runtime/validate_service_api_shutdown_abrupt_close_regression_live_contract_lane_impl.sh" ]; then
  echo "expected service api shutdown abrupt-close regression contract lane dispatcher to resolve implementation script" >&2
  exit 1
fi

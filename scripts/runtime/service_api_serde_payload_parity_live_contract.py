#!/usr/bin/env python3
"""Service API serde payload parity live policy contracts."""
...
REPORT_SCHEMA = "kamn.runtime.service-api-serde-payload-parity-live-validation.v1"
...
REQUIRED_REPORT_FIELDS = [
    ...
    "final_decision",
    "serde_dto_status",
    "serde_serializer_status",
    "serde_parser_status",
    "route_payload_parity_status",
    "fail_closed_status",
...
def _check_policy(args: argparse.Namespace) -> int:
    ...
    decision.reject_if(
        ...
        "service_api_serde_payload_policy_schema_mismatch",
    )
    decision.reject_if(
        observed_status not in {"pass", "fail"},
        "service_api_serde_payload_policy_status_invalid",
    )
    decision.reject_if(
        observed_final_decision not in {"GO", "NO-GO"},
        "service_api_serde_payload_policy_final_decision_invalid",
    )
    decision.reject_if(
        observed_final_decision != expected_final_decision,
        "service_api_serde_payload_policy_final_decision_mismatch",
    )

    for field_name in REQUIRED_VERIFIED_FIELDS:
        decision.reject_if(
            report.get(field_name) != "verified",
            f"service_api_serde_payload_policy_marker_missing:{field_name}",
        )

    decision.reject_if(
        report.get("fail_closed_reason_code") != EXPECTED_FAIL_CLOSED_REASON_CODE,
        "service_api_serde_payload_policy_fail_closed_reason_code_mismatch",
    )
    decision.reject_if(
        not _is_non_negative_int(report.get("elapsed_seconds")),
        "service_api_serde_payload_policy_elapsed_seconds_invalid",
    )
    ...
    policy_report: dict[str, Any] = {
        ...
        "service_api_serde_payload_policy_status": policy_status,
    ...
    print(f"service_api_serde_payload_policy_status={policy_status}")
    print(f"reason_codes={reason_codes_csv}")
    if output_json is not None:
        print(f"policy_report_file={output_json}")

    if final_decision != "GO":
        fail(f"service api serde payload parity live policy rejected: {reason_codes_csv}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Service API serde payload parity live policy checker contract."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    check_policy_parser = subparsers.add_parser(
        "check-policy",
        help="Validate service API serde payload parity live report policy.",
    )
    check_policy_parser.add_argument(
        "--report-file",
        required=True,
        help="Path to service API serde payload parity live validation report JSON.",
    )
    check_policy_parser.add_argument(
        "--expected-final-decision",
        default="GO",

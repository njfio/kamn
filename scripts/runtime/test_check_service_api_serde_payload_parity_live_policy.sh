...
report_file="$TMP_DIR/service-api-serde-payload-parity-live-summary.json"
...
if ! printf '%s\n' "$policy_output" | grep -q '^final_decision=GO$'; then
  echo "expected service api serde payload parity policy checker GO marker" >&2
  exit 1
fi
...
if payload.get("schema_version") != "kamn.runtime.service-api-serde-payload-parity-live-policy-report.v1":
...
if payload.get("service_api_serde_payload_policy_status") != "verified":
...
if [ "$tampered_code" -eq 0 ]; then
  echo "expected tampered service api serde payload parity report to fail policy checker" >&2
  exit 1
fi
if ! printf '%s\n' "$tampered_output" | grep -q 'service_api_serde_payload_policy_marker_missing:route_payload_parity_status'; then
  echo "expected deterministic mismatch reason code for tampered serde payload policy validation" >&2
  exit 1
fi

echo "service api serde payload parity live policy checker tests passed."

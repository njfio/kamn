...
if lane_payload.get("correlation_schema_status") != "verified":
    raise SystemExit("expected correlation_schema_status=verified")
if lane_payload.get("correlation_schema_version") != "kamn.runtime.unified-api-observability-correlation-schema.v1":
    raise SystemExit("expected deterministic correlation_schema_version marker in contract lane report")
if lane_payload.get("correlation_id_propagation_status") != "verified":
    raise SystemExit("expected deterministic correlation_id_propagation_status marker in contract lane report")

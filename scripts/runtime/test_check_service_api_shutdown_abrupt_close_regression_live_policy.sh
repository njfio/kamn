...
report_file="$TMP_DIR/service-api-shutdown-abrupt-close-regression-live-summary.json"
bash "$ROOT_DIR/scripts/lib/write_json_file.sh" "$report_file" <<'JSON'
{
  "schema_version": "kamn.runtime.service-api-shutdown-abrupt-close-regression-live-report.v1",
  "status": "pass",
  "final_decision": "GO",
  "lane_mode": "dry-run",
...
if ! printf '%s\n' "$policy_output" | grep -q '^final_decision=GO$'; then
  echo "expected service api shutdown abrupt-close regression policy checker GO marker" >&2
  exit 1
fi
...
if payload.get("schema_version") != "kamn.runtime.service-api-shutdown-abrupt-close-regression-live-policy-report.v1":
...
if payload.get("service_api_shutdown_abrupt_close_regression_policy_status") != "verified":
...
if [ "$tampered_code" -eq 0 ]; then
  echo "expected tampered service api shutdown abrupt-close regression report to fail policy checker" >&2
  exit 1
fi
if ! printf '%s\n' "$tampered_output" | grep -q 'service_api_shutdown_abrupt_close_regression_policy_marker_missing:abrupt_close_guard_status'; then
  echo "expected deterministic mismatch reason code for tampered shutdown abrupt-close regression policy validation" >&2
  exit 1
fi

echo "service api shutdown abrupt-close regression live policy checker tests passed."

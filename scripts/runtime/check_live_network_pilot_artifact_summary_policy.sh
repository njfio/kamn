...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/runtime/check_live_network_pilot_artifact_summary_policy.sh \
    --summary-file <path>
EOF
}
...
if [[ ! -f "$summary_file" ]]; then
  fail "summary file not found: $summary_file"
fi
...
for lane_key in ("smoke", "deep"):
    ...
    if not isinstance(lane_payload["elapsed_seconds"], int) or lane_payload["elapsed_seconds"] < 0:
...
if not isinstance(actual_reasons, list) or not all(isinstance(v, str) for v in actual_reasons):

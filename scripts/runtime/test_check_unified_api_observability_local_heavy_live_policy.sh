...
if payload.get("correlation_schema_version") != "kamn.runtime.unified-api-observability-correlation-schema.v1":
    raise SystemExit("expected deterministic correlation_schema_version marker in unified API-observability local-heavy policy report")
if payload.get("correlation_required_fields_csv") != "correlation_id,trace_id,trace_parent,span_id,request_id":
    raise SystemExit("expected deterministic correlation_required_fields_csv marker in unified API-observability local-heavy policy report")
if payload.get("correlation_id_propagation_status") != "verified":
    raise SystemExit("expected deterministic correlation_id_propagation_status marker in unified API-observability local-heavy policy report")
PY
...
propagation_mismatch_report="$TMP_DIR/unified-api-observability-local-heavy-summary.propagation-mismatch.json"
cp "$report_file" "$propagation_mismatch_report"
python3 - "$propagation_mismatch_report" <<'PY'
import json
import pathlib
import sys
...
path = pathlib.Path(sys.argv[1])
payload = json.loads(path.read_text(encoding="utf-8"))
payload["correlation_schema_version"] = "kamn.runtime.unified-api-observability-correlation-schema.v1"
payload["correlation_required_fields_csv"] = "correlation_id,trace_id,trace_parent,span_id,request_id"
payload["api_correlation_id"] = "corr-api-001"
payload["runtime_correlation_id"] = "corr-runtime-002"
payload["kolme_correlation_id"] = "corr-kolme-001"
payload["correlation_id_propagation_status"] = "mismatch"
path.write_text(json.dumps(payload, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY

set +e
propagation_mismatch_output="$(
  bash "$POLICY_CHECKER" \
    --report-file "$propagation_mismatch_report" \
    --expected-final-decision GO \
    --ci-fast-gate PASS \
    --output-json "$TMP_DIR/unified-api-observability-local-heavy-policy.propagation-mismatch.json" 2>&1
)"
propagation_mismatch_code=$?
set -e
if [ "$propagation_mismatch_code" -eq 0 ]; then
  echo "expected telemetry policy checker to fail closed for API/runtime/Kolme correlation propagation mismatch" >&2
  exit 1
fi
if ! printf '%s\n' "$propagation_mismatch_output" | grep -q 'unified_api_observability_local_heavy_policy_correlation_id_propagation_mismatch'; then
  echo "expected deterministic correlation propagation mismatch reason code for telemetry policy checker" >&2
  exit 1
fi

schema_drift_report="$TMP_DIR/unified-api-observability-local-heavy-summary.schema-drift.json"
cp "$report_file" "$schema_drift_report"
python3 - "$schema_drift_report" <<'PY'
import json
import pathlib
import sys
...
path = pathlib.Path(sys.argv[1])
payload = json.loads(path.read_text(encoding="utf-8"))
payload["correlation_schema_version"] = "kamn.runtime.unified-api-observability-correlation-schema.v0"
path.write_text(json.dumps(payload, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
...
schema_drift_output="$(
  bash "$POLICY_CHECKER" \
    --report-file "$schema_drift_report" \
    --expected-final-decision GO \
    --ci-fast-gate PASS \
    --output-json "$TMP_DIR/unified-api-observability-local-heavy-policy.schema-drift.json" 2>&1
)"
schema_drift_code=$?
set -e
if [ "$schema_drift_code" -eq 0 ]; then
  echo "expected telemetry policy checker to fail closed for correlation schema version drift" >&2
  exit 1
fi
if ! printf '%s\n' "$schema_drift_output" | grep -q 'unified_api_observability_local_heavy_policy_correlation_schema_version_mismatch'; then
  echo "expected deterministic correlation schema version mismatch reason code for telemetry policy checker" >&2
  exit 1
fi

required_fields_report="$TMP_DIR/unified-api-observability-local-heavy-summary.required-fields-mismatch.json"
cp "$report_file" "$required_fields_report"
python3 - "$required_fields_report" <<'PY'
import json
import pathlib
import sys
...
path = pathlib.Path(sys.argv[1])
payload = json.loads(path.read_text(encoding="utf-8"))
payload["correlation_required_fields_csv"] = "correlation_id,trace_id,trace_parent,span_id"
...
required_fields_output="$(
  bash "$POLICY_CHECKER" \
    --report-file "$required_fields_report" \
    --expected-final-decision GO \
    --ci-fast-gate PASS \
    --output-json "$TMP_DIR/unified-api-observability-local-heavy-policy.required-fields-mismatch.json" 2>&1
)"
required_fields_code=$?
set -e
if [ "$required_fields_code" -eq 0 ]; then
  echo "expected telemetry policy checker to fail closed for correlation required-field mismatch" >&2
  exit 1
fi
if ! printf '%s\n' "$required_fields_output" | grep -q 'unified_api_observability_local_heavy_policy_correlation_required_fields_mismatch'; then
  echo "expected deterministic correlation required-fields mismatch reason code for telemetry policy checker" >&2
  exit 1
fi

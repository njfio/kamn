...
prebuild_timeout_seconds="${KAMN_SERVICE_API_AXUM_INGRESS_PREBUILD_MAX_SECONDS:-900}"
if ! [[ "$prebuild_timeout_seconds" =~ ^[0-9]+$ ]] || [[ "$prebuild_timeout_seconds" -le 0 ]]; then
  echo "KAMN_SERVICE_API_AXUM_INGRESS_PREBUILD_MAX_SECONDS must be a positive integer" >&2
  exit 2
fi
prebuild_target_dir="${KAMN_SERVICE_API_AXUM_INGRESS_PREBUILD_TARGET_DIR:-$ROOT_DIR/target/service-api-axum-ingress}"
...
if [[ "$prebuild_code" -eq 124 ]]; then
  echo "service api axum ingress prebuild timed out" >&2
  exit 124
fi
if [[ "$prebuild_code" -ne 0 ]]; then
  echo "service api axum ingress prebuild failed" >&2
  exit "$prebuild_code"
fi
...
lane_output="$(
  KAMN_SERVICE_API_AXUM_INGRESS_SKIP_BUILD=1 \
    KAMN_SERVICE_API_AXUM_INGRESS_NODE_BIN="$prebuilt_node_bin" \

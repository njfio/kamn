...
fn spec_c08_cli_content_commands_execute_and_validate_args() {
    with_contract_server(4, |endpoint| {
        ...
        let expire_output = dispatch(&parsed(CommandKind::ExpireContent, endpoint, &["content-cli"]))
            .expect("expire-content should succeed");
        assert!(expire_output.text.contains("lifecycle_state=expired"));

        let tombstone_output =
            dispatch(&parsed(CommandKind::TombstoneContent, endpoint, &["content-cli"]))
                .expect("tombstone-content should succeed");
        assert!(tombstone_output.text.contains("redaction_status=redacted"));

        let query_output = dispatch(&parsed(CommandKind::QueryContent, endpoint, &["content-cli"]))
...
fn spec_c09_cli_bridge_commands_execute_and_validate_args() {
    with_contract_server(3, |endpoint| {
        ...
        let forward_output =
            dispatch(&parsed(CommandKind::ForwardBridgeMessage, endpoint, &["bridge-cli"]))
        ...
        let query_output = dispatch(&parsed(CommandKind::QueryBridgeMessage, endpoint, &["bridge-cli"]))
        ...
        for (command, label) in [
            (CommandKind::SubmitBridgeMessage, "submit_bridge_message_payload"),
            (CommandKind::ForwardBridgeMessage, "forward_bridge_message_id"),

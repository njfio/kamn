...
impl fmt::Display for DataLayerM5VectorIntegrationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidAgentDid { reason_code, detail } => {
            ...
            Self::InvalidVectorDimensions { expected, found } => {
                write!(f, "invalid vector dimensions: expected {expected}, found {found}")
            }
            ...
            Self::InsufficientAgentHistory { owner_did, agent_did } => {
                write!(f, "insufficient agent history for owner {owner_did}, agent {agent_did}")
            }
            Self::InvalidEmbeddingHashChain { owner_did, position, reason } => {
                write!(f, "invalid embedding hash chain for {owner_did} at {position}: {reason}")
            }
            Self::EmbeddingSequenceNotFound { owner_did, sequence } => {
                write!(f, "embedding sequence not found for owner {owner_did}: {sequence}")
            }
        }
    }
}

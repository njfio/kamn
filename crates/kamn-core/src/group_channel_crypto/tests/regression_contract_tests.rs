...
use super::super::{derive_group_aead_key, derive_group_aead_key_legacy, group_nonce_bytes, GroupChannelCryptoEngine, GroupChannelCryptoError, GROUP_MESSAGE_HKDF_BACKEND_MARKER, GROUP_MESSAGE_HMAC_BACKEND_MARKER};
...
fn regression_requires_constant_time_group_signature_compare() {
    ...
    assert!(
        !SOURCE.contains(["if expected_signature !=", " sealed.signature {"].concat().as_str())
    );
}
...
fn group_message_derivation_backend_markers_and_manual_helper_removal_contract() {
    assert_eq!(GROUP_MESSAGE_HKDF_BACKEND_MARKER, "rustcrypto.hkdf.sha256.v1");
    assert_eq!(GROUP_MESSAGE_HMAC_BACKEND_MARKER, "rustcrypto.hmac.sha256.v1");
...
fn group_nonce_bytes_do_not_expose_raw_counter_prefix() {
    ...
    assert_ne!(&group_nonce_bytes("kamn:did:agent:alice", 7, nonce)[..8], &nonce.to_le_bytes());
}

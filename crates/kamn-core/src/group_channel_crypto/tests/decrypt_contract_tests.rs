use super::support::{decode_ciphertext_parts, legacy_raw_nonce, legacy_v1_ciphertext, with_key_agreement_seed, TEST_KEY_SEED_HEX};
use super::super::{derive_group_aead_key, derive_group_shared_secret, GroupChannelCryptoEngine, GroupChannelCryptoError};
...
fn decrypt_rejects_unauthorized_recipient() {
    with_key_agreement_seed(Some(TEST_KEY_SEED_HEX), || {
        ...
        let distribution = engine
            ...
            .unwrap();
        let sealed = engine.encrypt("kamn:did:agent:alice", "group payload", 35).unwrap();
        assert_eq!(
...
fn decrypt_rejects_tampered_signature() {
    with_key_agreement_seed(Some(TEST_KEY_SEED_HEX), || {
        ...
        engine
            ...
            .unwrap();
        let mut sealed = engine.encrypt("kamn:did:agent:alice", "group payload", 37).unwrap();
        let replacement = if sealed.signature.starts_with('0') { '1' } else { '0' };
        sealed.signature.replace_range(0..1, &replacement.to_string());
...
fn decrypt_rejects_zero_nonce_fail_closed() {
    with_key_agreement_seed(Some(TEST_KEY_SEED_HEX), || {
        ...
        engine
            ...
            .unwrap();
        let mut sealed = engine.encrypt("kamn:did:agent:alice", "group payload", 39).unwrap();
        sealed.nonce = 0;
        assert_eq!(
...
fn decrypt_accepts_legacy_v1_sha256_kdf_ciphertext_for_compatibility() {
    with_key_agreement_seed(Some(TEST_KEY_SEED_HEX), || {
        ...
        assert_eq!(engine.decrypt(recipient_did, &sealed).unwrap(), "legacy-group-v1");
    });
}
...
fn encrypt_output_does_not_authenticate_under_legacy_raw_prefix_nonce_layout() {
    with_key_agreement_seed(Some(TEST_KEY_SEED_HEX), || {
        ...
        let distribution = engine
            .distribute_sender_key(sender_did, sender_key_ref, vec![recipient_did.to_owned()])
            .unwrap();
        let sealed = engine.encrypt(sender_did, "group-nonce-layout", nonce).unwrap();
        let master_seed = super::super::load_key_agreement_master_seed().unwrap();
        let shared_secret = derive_group_shared_secret(channel_id, sender_key_ref, distribution.key_generation, &master_seed);
        let aead_key = derive_group_aead_key(&shared_secret, channel_id, distribution.key_generation).unwrap();
        let cipher = XChaCha20Poly1305::new((&aead_key).into());
        let xnonce = XNonce::from(legacy_raw_nonce(sender_did, distribution.key_generation, nonce));
        let decrypted = cipher.decrypt(&xnonce, Payload { msg: &decode_ciphertext_parts(&sealed), aad: &[] });

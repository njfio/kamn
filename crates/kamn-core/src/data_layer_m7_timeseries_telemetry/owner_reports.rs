...
pub(crate) fn project_owner_billing_daily(
    ...
    authorize_owner_scope(query.requester_owner_did.as_str(), query.owner_did.as_str(), DATA_LAYER_M7_OWNER_SCOPE_DENIED_REASON_CODE)?;
    Ok(project_m7_owner_billing_daily_rows(query.owner_did.as_str(), owner_points))
}
...
pub(crate) fn reconcile_owner_billing_daily(
    ...
    let extracted_projections = project_m7_owner_billing_daily_projection(input.owner_did.as_str(), owner_points);
    let extracted_report = reconcile_data_layer_m7_owner_billing_daily(&extracted_projections, ExtractedBillingInput {
...
pub(crate) fn evaluate_owner_observability(
    ...
    for point in owner_points {
        ...
        let report = monitor.evaluate(sample).map_err(map_observability_error_to_timeseries)?;
        reports.push(report);
    }

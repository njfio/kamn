...
pub fn map_content_store_validation_error(error: ContentStorageError) -> ConfigError {
    match error {
        ...
        ContentStorageError::Io(detail) => {
            io_error("content-storage", "content_storage_io_error", detail)
        }
        other => compatibility_error(
            "content-storage",
            "content_storage_compatibility_failed",
            other,
        ),
    }
}
...
pub fn map_did_registry_store_validation_error(error: DidRegistryError) -> ConfigError {
    match error {
        ...
        DidRegistryError::PersistenceIo(detail) => {
            io_error("did-registry", "did_registry_io_error", detail)
        }

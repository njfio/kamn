...
fn regression_bootstrap_fails_closed_when_content_store_payload_is_corrupt() {
    assert_corrupt_payload(
        CONTENT_STORE_FIXTURE,
...
fn regression_bootstrap_fails_closed_when_message_snapshot_payload_is_corrupt() {
    assert_corrupt_payload(
        MESSAGE_LIFECYCLE_STORE_FIXTURE,
        "schema|1\nbroken\n",
...
fn assert_corrupt_payload(
    ...
    assert!(
        matches!(result, Err(ConfigError::RuntimeStoreCorruptPayload { store: actual_store, reason_code: actual_reason, .. }) if actual_store == store && actual_reason == reason_code)
    );
}
...
fn assert_schema_error(
    ...
    let result = bootstrap(config_for(&storage_dir));
    assert!(
        matches!(result, Err(ConfigError::RuntimeStoreSchemaIncompatible { store: actual_store, reason_code: actual_reason, expected: actual_expected, found: actual_found }) if actual_store == store && actual_reason == reason_code && actual_expected == expected && actual_found == found)
    );
}

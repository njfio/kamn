...
fn range_mode_reports_head_precedes_activation_base_for_older_heads() {
    ...
    let (_output, report) = run_range_checker(repo.root(), &base, &first, "6840-head-precedes-report");

    assert_eq!(string_field(&report, "status"), "ok");
    assert_eq!(string_field(&report, "activation_scope_status"), "head_precedes_activation_base");

...
fn regression_budget_guard_marker_is_documented() {
    ...
    // Regression: #876
    assert!(LIVE_NETWORK_WAVE_DOC.contains("`Regression: #876`"));
    assert!(LIVE_NETWORK_WAVE_DOC.contains("signature mismatch and timeout reason codes"));
    // Regression: #878
    ...
    // Regression: #1170
    assert!(LIVE_NETWORK_WAVE_DOC.contains("`Regression: #1170`"));
    assert!(LIVE_NETWORK_WAVE_DOC.contains(
        "integration policy checker wrapper remains pinned to shared contract implementation marker"
    ));
    // Regression: #1174

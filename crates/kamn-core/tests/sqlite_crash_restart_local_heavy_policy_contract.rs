...
const POLICY_REASON_CODES_CSV: &str = "sqlite_crash_restart_policy_required_field_missing,sqlite_crash_restart_policy_marker_mismatch,sqlite_crash_restart_policy_reason_taxonomy_mismatch,sqlite_crash_restart_policy_profile_contract_mismatch,sqlite_crash_restart_policy_runbook_marker_parity_mismatch,sqlite_crash_restart_policy_strategy_marker_parity_mismatch,ci_fast_gate_failed,sqlite_crash_restart_policy_expected_decision_mismatch,sqlite_crash_restart_policy_violation";
    ...
    run_command(
        {
            let mut command = Command::new("bash");
            command
                .arg("scripts/runtime/check_sqlite_crash_restart_local_heavy_policy.sh")
                .arg("--report-file")
                .arg(report_file)
                ...
                .arg("--output-json")
                .arg(output_json);
            if let Some(path) = runbook_file {
                command.arg("--runbook-file").arg(path);
            }
        ...
        "sqlite crash-restart local-heavy policy checker",
    )
}

#[test]
fn unit_sqlite_crash_restart_local_heavy_policy_checker_accepts_valid_report() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-unit");
    let lane_report = tmp.path().join("lane-report.json");
    let policy_report = tmp.path().join("policy-report.json");

    let lane_output = run_lane("combined", &lane_report);
    assert_success(&lane_output, "generate baseline crash-restart lane report");

    let policy_output = run_policy(&lane_report, &policy_report, None, None);
    assert_success(
        &policy_output,
        "sqlite crash-restart policy checker baseline",
    );

    let text = output_text(&policy_output);
    assert!(text.contains("status=pass"));
    assert!(text.contains("final_decision=GO"));
    assert!(text.contains("sqlite_crash_restart_policy_status=verified"));
    assert!(text.contains("sqlite_crash_restart_runbook_marker_parity_status=verified"));
    assert!(text.contains("sqlite_crash_restart_strategy_marker_parity_status=verified"));
...
#[test]
fn functional_sqlite_crash_restart_local_heavy_policy_checker_rejects_tampered_marker() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-functional");
    ...
    let tampered_payload = baseline_payload.replacen(
        ...
        "\"corruption_drill_status\": \"failed\"",
        1,
    );
    assert_ne!(
        baseline_payload, tampered_payload,
        "tampered payload should mutate corruption marker"
    );
    ...
    assert_failure(
        &policy_output,
        "sqlite crash-restart policy checker with tampered corruption marker",
    );
    assert!(
        output_text(&policy_output).contains("sqlite_crash_restart_policy_marker_mismatch"),
        "tampered marker must fail closed with deterministic marker mismatch reason"
    );
}

#[test]
fn integration_sqlite_crash_restart_local_heavy_policy_checker_enforces_docs_and_runbook_parity() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-integration");
    ...
    assert_success(
        &policy_output,
        "sqlite crash-restart policy checker integration baseline",
    );
    let text = output_text(&policy_output);
    assert!(text.contains("sqlite_crash_restart_runbook_marker_parity_status=verified"));
    assert!(text.contains("sqlite_crash_restart_strategy_marker_parity_status=verified"));
...
#[test]
fn regression_sqlite_crash_restart_local_heavy_policy_checker_rejects_runbook_parity_drift() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-regression-runbook");
    let lane_report = tmp.path().join("lane-report-regression-runbook.json");
    let policy_report = tmp.path().join("policy-report-regression-runbook.json");
    let drifted_runbook = tmp.path().join("kolme-devnet-ops-drifted.md");

    let lane_output = run_lane("combined", &lane_report);
    assert_success(&lane_output, "generate baseline crash-restart lane report");

    let runbook_text = load_text(&repo_path("docs/deploy/kolme_devnet_ops.md"));
    let drifted_runbook_text = runbook_text.replacen(
        "sqlite_crash_restart_runbook_marker_parity_status=verified",
        "sqlite_crash_restart_runbook_marker_parity_status=drifted",
        1,
    );
    assert_ne!(
        runbook_text, drifted_runbook_text,
        "runbook drift fixture should mutate parity marker"
    );
    write_text(&drifted_runbook, &drifted_runbook_text);

    let policy_output = run_policy(
        &lane_report,
        &policy_report,
        Some(drifted_runbook.as_path()),
        None,
    );
    assert_failure(
        &policy_output,
        "sqlite crash-restart policy checker with drifted runbook parity markers",
    );
    assert!(
        output_text(&policy_output)
            .contains("sqlite_crash_restart_policy_runbook_marker_parity_mismatch"),
        "drifted runbook markers must fail closed with deterministic reason"
    );
}

#[test]
fn regression_sqlite_crash_restart_local_heavy_policy_checker_rejects_strategy_parity_drift() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-regression-strategy");
    ...
    assert_failure(
        &policy_output,
        "sqlite crash-restart policy checker with drifted strategy parity markers",
    );
    assert!(
        output_text(&policy_output)
            .contains("sqlite_crash_restart_policy_strategy_marker_parity_mismatch"),
        "drifted strategy markers must fail closed with deterministic reason"
    );
}

#[test]
fn performance_sqlite_crash_restart_local_heavy_policy_checker_stays_within_budget() {
    let tmp = TempDir::new("sqlite-crash-restart-policy-performance");
    ...
    assert_success(
        &policy_output,
        "sqlite crash-restart policy checker performance baseline",
    );
    let elapsed = started.elapsed();
    assert!(
        elapsed.as_secs() <= 5,
        "sqlite crash-restart policy checker exceeded 5s budget window: {elapsed:?}"
    );
}

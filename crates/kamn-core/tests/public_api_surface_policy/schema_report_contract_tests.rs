...
fn public_api_surface_report_schema_is_deterministic() {
    ...
    assert!(rendered.contains(&format!("policy_status={policy_status}")));
    assert!(rendered.contains(&format!("reason_codes={reason_codes}")));
    assert!(rendered.contains(&format!("warn_total_delta_max={warn_total_delta_max}")));
    assert!(rendered.contains(&format!("fail_total_delta_max={fail_total_delta_max}")));
}

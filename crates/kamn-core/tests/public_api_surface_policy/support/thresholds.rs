...
pub(crate) fn load_thresholds(path: &Path) -> PolicyThresholds {
    if !path.is_file() {
        fail("threshold_fixture_missing", &format!("missing threshold fixture {}", path.display()));
    }
    let map = parse_key_value_fixture(&read_file(path, "threshold_fixture_missing"), "threshold_fixture_invalid");
...
pub(crate) fn load_waiver(path: &Path) -> PolicyWaiver {
    let map = parse_key_value_fixture(&read_file(path, "waiver_fixture_invalid"), "waiver_fixture_invalid");
    assert_waiver_schema(&map, path);
    let mitigation_issue = required_value(&map, "mitigation_issue", "waiver_missing_mitigation_issue");
    ...
    if max_total_delta < 0 {
        fail("waiver_fixture_invalid", &format!("max_total_delta must be non-negative in {}", path.display()));
    }
    PolicyWaiver { mitigation_issue: mitigation_issue.to_owned(), max_total_delta }
}
...
fn assert_threshold_schema(map: &BTreeMap<String, String>, path: &Path) {
    ...
    if schema_version != THRESHOLD_SCHEMA_VERSION {
        fail("threshold_schema_mismatch", &format!("unexpected schema {} in {}", schema_version, path.display()));
    }
}
...
fn assert_waiver_schema(map: &BTreeMap<String, String>, path: &Path) {
    ...
    if schema_version != WAIVER_SCHEMA_VERSION {
        fail("waiver_schema_mismatch", &format!("unexpected waiver schema {} in {}", schema_version, path.display()));
    }
}
...
fn assert_mitigation_issue(mitigation_issue: &str, path: &Path) {
    ...
    if !valid {
        fail(
            ...
            &format!("mitigation_issue must be #<digits>, got {} in {}", mitigation_issue, path.display()),
        );
    }
}

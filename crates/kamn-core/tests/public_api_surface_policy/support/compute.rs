...
pub(crate) fn compute_report_with_policy() -> (
    ...
    let modules = parse_public_modules(&read_file(&repo_path("src/lib.rs"), "module_source_missing"));
    ...
    let report = build_report(&modules, baseline_total_public_items, &baseline_module_public_items);

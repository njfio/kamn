use super::super::{AUDIT_INTEGRITY_REASON_CODES_CSV, AUDIT_INTEGRITY_REASON_TAXONOMY_VERSION, DOC, LIFECYCLE_CI_DRY_RUN_REASON_CODES_CSV, LIFECYCLE_CI_DRY_RUN_REASON_TAXONOMY_VERSION};
...
fn doc_contains_audit_integrity_dry_run_governance_markers() {
    ...
    for reason_code in AUDIT_INTEGRITY_REASON_CODES_CSV.split(',') {
        assert!(DOC.contains(reason_code), "missing audit-integrity fail-closed reason marker {reason_code}");
    }
...
fn doc_contains_lifecycle_ci_dry_run_governance_markers() {
    ...
    for reason_code in LIFECYCLE_CI_DRY_RUN_REASON_CODES_CSV.split(',') {
        assert!(DOC.contains(reason_code), "missing lifecycle ci dry-run reason marker {reason_code}");
        assert!(DOC.contains(&format!("lifecycle_ci_dry_run_remediation.{reason_code}=")), "missing lifecycle ci dry-run remediation marker {reason_code}");
    }

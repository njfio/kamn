...
fn service_api_ops_configuration_contains_journal_wal_partial_write_fault_injection_markers() {
    assert!(DOC.contains("## Journal/WAL Partial-Write Fault Injection Contracts (Issue #4016)"));
    assert!(DOC.contains(
    ...
    assert!(DOC.contains("partial_snapshot_file_write -> recovery_clean"));
    assert!(DOC.contains("partial_journal_tail_write -> fail_closed_corrupt_tail"));
    assert!(DOC.contains("partial_snapshot_without_journal -> recovery_repaired_corrupt_payload"));
    assert!(DOC.contains(
    ...
    assert!(DOC.contains("Regression: #4016"));
}
...
fn service_api_ops_configuration_contains_live_postgres_daemon_runtime_matrix_parallel_lane_topology_host_mode_host_pair_lane_set_lane_fingerprint_hash_order_normalization_digest_markers(
    ...
    assert!(DOC.contains("Regression: #5400"));
}
...
fn service_api_ops_configuration_contains_daemon_tests_live_postgres_fixture_decomposition_markers()
{
    ...
    assert!(DOC.contains("daemon_tests_live_postgres_fixture_phase3_root_target_max_lines=300"));
    assert!(DOC.contains(
    ...
    assert!(DOC.contains("Regression: #5402, #5418, #5420"));
}

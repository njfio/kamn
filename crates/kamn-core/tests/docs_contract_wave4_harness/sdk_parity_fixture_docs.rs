...
fn rust_doc_references_sdk_schema_shared_contract_script() {
    assert_sdk_schema_contract_markers();
    assert_regression_markers();
    assert_shared_wrapper_markers();
}

fn assert_sdk_schema_contract_markers() {
...
fn assert_regression_markers() {
    for marker in [
        ...
        assert!(RUST_DOC.contains(marker));
    }
}

fn assert_shared_wrapper_markers() {
    assert!(

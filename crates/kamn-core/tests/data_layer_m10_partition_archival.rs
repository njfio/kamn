...
fn spec_c24_phase6_scheduler_cycle_deferred_path_returns_no_execution_side_effects() {
    let owner_did = "kamn:did:owner:phase6-scheduler-deferred";
    let mut m8_registry = DataLayerM8ComplianceRegistry::new();
    let mut m10_registry = DataLayerM10PartitionLifecycleRegistry::new();
    m10_registry
        .register_partition(partition_input(202601, false))
    ...
    let mut recent_message = m8_message_input(owner_did, "defer-message", 1_699_999_990);
    recent_message.retention_class = DataLayerM8RetentionClass::Ephemeral;
    m8_registry
        .register_message(recent_message)
        .expect("message should register");
    ...
    let cycle_report = data_layer_m10_execute_phase6_scheduler_cycle(
        ...
        DataLayerM10Phase6SchedulerCycleRequest {
            ...
            execution_request: phase6_request(
                owner_did,
                BTreeMap::from([(202601, vec!["defer-message".to_owned()])]),
            ),
        },
    )
    ...
    let message = m8_registry
        .message_for_owner(owner_did, "defer-message")
...
fn spec_c25_phase6_scheduler_cycle_preflight_budget_overflow_fails_closed_before_execution() {
    let owner_did = "kamn:did:owner:phase6-scheduler-preflight";
    let mut m8_registry = DataLayerM8ComplianceRegistry::new();
    let mut m10_registry = DataLayerM10PartitionLifecycleRegistry::new();
    m10_registry
        .register_partition(partition_input(202401, false))
...
fn spec_c26_phase6_scheduler_cycle_triggered_executes_within_budget_evidence() {
    let owner_did = "kamn:did:owner:phase6-scheduler-triggered";
    let mut m8_registry = DataLayerM8ComplianceRegistry::new();
    let mut m10_registry = DataLayerM10PartitionLifecycleRegistry::new();
    m10_registry
        .register_partition(partition_input(202401, false))
...
impl DataLayerM10Phase6CompliancePort for FakePhase6Port {
    ...
    ) -> Result<DataLayerM10ComplianceProjectionMessageState, DataLayerM10Phase6CompliancePortError>
    {
        ...
        self.message_states
            .get(message_id)
            .cloned()
            .ok_or_else(|| DataLayerM10Phase6CompliancePortError::LookupFailed("missing".to_owned()))
    }
}
...
fn spec_c38_phase6_orchestration_with_port_supports_seam_without_direct_m8_registry_argument() {
    ...
    assert_eq!(report.reason_code, DATA_LAYER_M10_PHASE6_EXECUTION_APPLIED_REASON_CODE);

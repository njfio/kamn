...
fn regression_transport_fed_restart_replay_tamper_matrix_emits_deterministic_reason_codes() {
    ...
    let tamper_cases = vec![
        (vec![], "canonical_replay_checkpoint_missing".to_owned(), "missing checkpoint"),
        (vec![sample_canonical_record(99, "digest-31", "tx-31")], "canonical_replay_block_height_mismatch".to_owned(), "height drift"),
        (vec![sample_canonical_record(31, "digest-31-tampered", "tx-31")], "canonical_replay_payload_digest_mismatch".to_owned(), "payload digest drift"),
        (vec![CanonicalCommitRecord { transaction_ids: vec!["tx-31-tampered".to_owned()], ..baseline.clone() }], "canonical_replay_transaction_ids_mismatch".to_owned(), "transaction id drift"),
    ];

    for (tampered_post_restart, expected_reason_code, case_name) in tamper_cases {
        let error = build_canonical_replay_evidence_bundle(&pre_restart, &tampered_post_restart).expect_err("tampered replay lineage must fail closed");
        assert!(matches!(error, BlockPipelineError::ReplayDrift { reason_code, .. } if reason_code == expected_reason_code), "unexpected reason code for tamper case: {case_name}");
    }
}
...
fn build_restart_replay_pipeline(
    ...
    let pipeline = build_duplicate_replay_pipeline(path, pre_restart.as_slice(), transport, recipient, topic);

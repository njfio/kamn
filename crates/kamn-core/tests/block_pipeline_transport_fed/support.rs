pub(crate) use kamn_core::{
    ...
    BlockPipelineError, CanonicalCandidateOutcome, CanonicalCommitRecord,
    CanonicalCommitStore, CanonicalReplayEvidenceBundle,
    ...
    AcceptAllForkChoiceHook, CanonicalCandidateDecision,
};
...
pub(crate) fn sample_consensus_input() -> BlockConsensusRoundInput {
    BlockConsensusRoundInput {
        ...
        listener_votes: vec![("kamn:did:agent:listener-alpha".to_owned(), "att-1".to_owned())],
        approver_votes: vec![(
...
pub(crate) fn build_valid_chain_transactions() -> Vec<BaselineTransaction> {
    ...
    planner.submit_transaction(tx_one.clone()).expect("first tx should be accepted");
...
pub(crate) fn sample_canonical_record(height: u64, digest: &str, tx_id: &str) -> CanonicalCommitRecord {

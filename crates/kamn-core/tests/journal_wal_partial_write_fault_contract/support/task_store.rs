...
use kamn_core::{
    FileTaskOperationSnapshotStore, TaskOperationSnapshotStore, TaskOperationSnapshotStoreError,
};

pub(crate) fn run(case: &FixtureCase, temp_dir: &TempDir) {
    let paths = prepare_store_paths(temp_dir, "task", &case.case_id);
...
fn run_clean_recovery(case: &FixtureCase, snapshot_path: std::path::PathBuf) {
    ...
    let mut store =
        FileTaskOperationSnapshotStore::new(snapshot_path.clone()).expect("store should build");

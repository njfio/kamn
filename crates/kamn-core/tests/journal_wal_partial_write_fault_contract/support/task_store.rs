...
use super::snapshot_fixtures::task_snapshots;
use super::{FixtureCase, TempDir};
use kamn_core::{FileTaskOperationSnapshotStore, TaskOperationSnapshotStore, TaskOperationSnapshotStoreError};

pub(crate) fn run(case: &FixtureCase, temp_dir: &TempDir) {
    let snapshot_path = temp_dir.path().join(format!("task-{}.snapshot", case.case_id));
...
fn run_clean_recovery(case: &FixtureCase, snapshot_path: std::path::PathBuf) {
    ...
    let mut store = FileTaskOperationSnapshotStore::new(snapshot_path.clone()).expect("store should build");
...
fn run_corrupt_tail(case: &FixtureCase, snapshot_path: std::path::PathBuf, journal_path: std::path::PathBuf) {
    ...
    let mut store = FileTaskOperationSnapshotStore::new(snapshot_path).expect("store should build");

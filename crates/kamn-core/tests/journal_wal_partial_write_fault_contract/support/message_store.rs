...
use kamn_core::{FileMessageLifecycleSnapshotStore, MessageLifecycleSnapshotStore, MessageLifecycleSnapshotStoreError};

pub(crate) fn run(case: &FixtureCase, temp_dir: &TempDir) {
    let snapshot_path = temp_dir.path().join(format!("message-{}.snapshot", case.case_id));
...
fn run_clean_recovery(case: &FixtureCase, snapshot_path: std::path::PathBuf) {
    ...
    let mut store = FileMessageLifecycleSnapshotStore::new(snapshot_path.clone()).expect("store should build");
...
fn run_corrupt_tail(case: &FixtureCase, snapshot_path: std::path::PathBuf, journal_path: std::path::PathBuf) {
    ...
    let mut store = FileMessageLifecycleSnapshotStore::new(snapshot_path).expect("store should build");

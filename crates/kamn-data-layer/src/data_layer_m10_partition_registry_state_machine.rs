...
use crate::{
    ...
    data_layer_m10_validate_partition_month_id,
};
...
impl DataLayerM10PartitionRegistryStateMachine {
    ...
    pub fn register_partition(
        ...
        if self.partitions.contains_key(&input.partition_month_id) {
            return Err(Self::duplicate_partition_error(input.partition_month_id));
        }
    ...
    pub fn apply_partition_shred_completeness(
        ...
        let record = self
            .partitions
            .get_mut(&partition_month_id)
            .ok_or_else(|| DataLayerM10PartitionRegistryStateMachineError::PartitionNotFound(partition_name))?;
    ...
    pub fn plan_future_partition_names(
        ...
        for offset in 1..=u32::from(months_ahead) {
            let month_id =
                data_layer_m10_add_months(reference_month_id, offset).map_err(map_partition_month_error)?;
            result.push(data_layer_m10_format_partition_name(month_id).map_err(map_partition_month_error)?);
        }
        Ok(result)
    }
    ...
    ) -> Result<Vec<DataLayerM10ArchivalIndexEntry>, DataLayerM10PartitionRegistryStateMachineError>
    {
        ...
        for record in self.partitions.values_mut() {
            if !record_is_due_for_archive(record, request.now_month_id, request.active_retention_months)? {
                continue;
            }
    ...
    pub fn reattach_partition(
        ...
        if record.lifecycle_status != DataLayerM10PartitionStatus::Archived {
            return Err(DataLayerM10PartitionRegistryStateMachineError::InvalidLifecycleTransition {
                partition_name: record.partition_name.clone(),
...
fn validate_partition_month_id(
    ...
    data_layer_m10_validate_partition_month_id(partition_month_id).map_err(map_partition_month_error)
}
...
fn validate_non_empty(
    ...
    if value.trim().is_empty() {
        return Err(DataLayerM10PartitionRegistryStateMachineError::EmptyField(field));
    }
    Ok(())
}

fn record_is_due_for_archive(
...
fn map_partition_month_error(
    error: crate::DataLayerM10PartitionMonthPolicyError,
) -> DataLayerM10PartitionRegistryStateMachineError {
    match error {
        crate::DataLayerM10PartitionMonthPolicyError::InvalidPartitionMonthId(value) => {
            DataLayerM10PartitionRegistryStateMachineError::InvalidPartitionMonthId(value)
        }
    }
}

fn missing_partition(partition_name: &str) -> DataLayerM10PartitionRegistryStateMachineError {

...
use kamn_data_layer::{
    ...
    DataLayerM10ComplianceShredProjectionRequest, DataLayerM10PartitionRecordInput,
    DataLayerM10PartitionRegistryStateMachine, DATA_LAYER_M10_COMPLIANCE_INPUT_INVALID_REASON_CODE,
...
impl FakeProjectionPort {
    fn with_messages(messages: Vec<(&str, bool, Option<u64>)>) -> Self {
        let messages = messages
            .into_iter()
            .map(
                |(message_id, legal_hold_active, shredded_at_epoch_seconds)| {
                    (
                        message_id.to_owned(),

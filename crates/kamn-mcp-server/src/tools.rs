...
const TASK_PAYLOAD_INPUT_SCHEMA_JSON: &str = r#"{"type":"object","required":["task_id","payload"],"properties":{"task_id":{"type":"string"},"payload":{"type":"string"}},"additionalProperties":false}"#;
...
fn tool_descriptor_for_name(name: &'static str) -> ToolDescriptor {
    ...
    let input_schema = match name {
        ...
        "accept_task" | "complete_task" => TASK_PAYLOAD_INPUT_SCHEMA_JSON,
        "fund_escrow" => PAYLOAD_INPUT_SCHEMA_JSON,
        "release_escrow" => ESCROW_PAYLOAD_INPUT_SCHEMA_JSON,

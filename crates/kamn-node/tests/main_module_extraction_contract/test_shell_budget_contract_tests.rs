...
fn main_module_extraction_contract_main_tests_decomposition_and_budget_markers_remain_stable() {
    ...
    assert!(
        main_tests_rs.contains("main_tests structural budget shell only; keep domain tests in src/main_tests/*.rs"),
...
fn main_module_extraction_contract_daemon_tests_decomposition_shell_markers_remain_stable() {
    ...
    assert!(
        daemon_tests_rs.contains("daemon_tests structural budget shell phase3; route runtime/matrix/topology contracts"),
    ...
    assert!(daemon_tests_rs.contains("include!(\"daemon_tests/live_postgres_matrix_contract_tests.rs\");"));
    assert!(daemon_tests_rs.contains("include!(\"daemon_tests/live_postgres_topology_contract_tests.rs\");"));
    assert!(daemon_tests_rs.contains("include!(\"daemon_tests/live_postgres_distributed_execution_contract_tests.rs\");"));
    ...
    assert!(!daemon_tests_rs.contains("fn functional_runtime_daemon_emits_structured_transition_markers("));

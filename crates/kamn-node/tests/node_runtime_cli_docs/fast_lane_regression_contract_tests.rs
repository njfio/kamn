...
fn doc_contains_fast_and_cost_effective_validation_lane() {
    assert_doc_markers(DOC, FAST_VALIDATION_MARKERS, "node runtime CLI fast validation lane");
}

#[test]
fn doc_contains_docs_fast_lane_command_checks() {
    assert_doc_markers(DOC, DOCS_FAST_LANE_MARKERS, "node runtime CLI docs fast lane");
}

#[test]
fn doc_contains_daemon_focused_fast_lane_commands() {
    assert_doc_markers(DOC, DAEMON_FAST_LANE_MARKERS, "node runtime CLI daemon fast lane");
}

#[test]
fn regression_requires_invalid_output_mode_rule() {
    assert_doc_markers(DOC, INVALID_OUTPUT_MODE_RULE, "node runtime CLI invalid output regression");
}

#[test]
fn regression_requires_invalid_profile_rule() {
    assert_doc_markers(DOC, INVALID_PROFILE_RULE, "node runtime CLI invalid profile regression");
}

#[test]
fn regression_requires_invalid_diagnostics_mode_rule() {
    assert_doc_markers(DOC, INVALID_DIAGNOSTICS_RULE, "node runtime CLI invalid diagnostics regression");
}

#[test]
fn regression_requires_runtime_planning_candidate_rules() {
    assert_doc_markers(DOC, PLANNING_REGRESSION_RULE, "node runtime CLI planning regression");
}

#[test]
fn regression_requires_runtime_recovery_rejection_rules() {
    assert_doc_markers(DOC, RECOVERY_REJECTION_RULE, "node runtime CLI recovery rejection regression");
}

#[test]
fn regression_requires_runtime_recovery_error_rule_references() {
    assert_doc_markers(DOC, RECOVERY_ERROR_RULES, "node runtime CLI recovery error regressions");
}

#[test]
fn regression_requires_runtime_daemon_control_rules() {
    assert_doc_markers(DOC, DAEMON_CONTROL_RULE, "node runtime CLI daemon control regression");
}

#[test]
fn regression_requires_runtime_daemon_lifecycle_rules() {
    assert_doc_markers(DOC, DAEMON_LIFECYCLE_RULE, "node runtime CLI daemon lifecycle regression");
}

#[test]
fn regression_requires_runtime_daemon_lease_guard_rules() {
    assert_doc_markers(DOC, DAEMON_LEASE_GUARD_RULE, "node runtime CLI daemon lease regression");
}

#[test]
fn regression_requires_runtime_kolme_live_guard_rules() {
    assert_doc_markers(DOC, KOLME_LIVE_GUARD_RULE, "node runtime CLI kolme-live regression");
}

#[test]
fn regression_requires_runtime_kolme_live_provider_drift_guard_rules() {
    assert_doc_markers(DOC, KOLME_PROVIDER_DRIFT_RULE, "node runtime CLI provider drift regression");
}

...
const TRANSPORT_SUPPORT_FILE: &str =
    "src/main_tests/observability_endpoint_tests/support/transport_support.rs";
const ASYNC_REGRESSION_FILE: &str =
...
const TRANSPORT_SUPPORT_MARKERS: &[&str] =
    &["use std::io::{ErrorKind, Read, Write};", "use std::thread;"];
...
fn regression_observability_endpoint_support_modules_restore_explicit_std_imports() {
    assert_file_contains_markers(
        SUPPORT_FILE,
        &["use std::net::TcpListener;", "use std::thread;"],
    );

...
pub(super) fn text_opt_list(value: &Option<Vec<String>>) -> String {
    value.as_ref().map(|items| text_list(items)).unwrap_or_else(|| "none".to_owned())
}
...
pub(super) fn json_opt_str(value: &Option<String>) -> String {
    value.as_deref().map(json_str).unwrap_or_else(|| "null".to_owned())
}

pub(super) fn json_opt_num<T: Display>(value: Option<T>) -> String {
    value.map(|item| item.to_string()).unwrap_or_else(|| "null".to_owned())
}

pub(super) fn json_opt_list(value: &Option<Vec<String>>) -> String {
    value.as_ref().map(|items| json_list(items)).unwrap_or_else(|| "null".to_owned())
}

pub(super) fn json_list(items: &[String]) -> String {
    format!("[{}]", items.iter().map(|item| json_str(item)).collect::<Vec<_>>().join(","))
}

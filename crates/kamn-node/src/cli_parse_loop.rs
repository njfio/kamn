...
pub(super) fn parse_layered_args_into_state(
    ...
    while let Some(arg) = iter.next() {
        if try_parse_known_option(arg.as_str(), &mut iter, state)? {
            continue;
        }
...
fn try_parse_known_option(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    if try_parse_core_common_arg(arg, iter, state)? {
        return Ok(true);
    }
    if try_parse_daemon_arg(arg, iter, state)? {
        return Ok(true);
    }
    if try_parse_endpoint_arg(arg, iter, state)? {
        return Ok(true);
    }
    if try_parse_kolme_live_arg(arg, iter, state)? {
        return Ok(true);
    }
    if try_parse_planning_recovery_arg(arg, iter, state)? {
        return Ok(true);
    }
    Ok(false)
}

fn try_parse_core_common_arg(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    try_parse_core_common_option(
        arg,
        iter,
...
fn try_parse_daemon_arg(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    try_parse_daemon_option(
        arg,
        iter,
...
fn try_parse_endpoint_arg(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    try_parse_endpoint_option(
        arg,
        iter,
        &mut EndpointOptionState {
            ...
            api_rate_limit_per_second_overridden: &mut state.api_rate_limit_per_second_overridden,
...
fn try_parse_kolme_live_arg(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    try_parse_kolme_live_option(
        arg,
        iter,
...
fn try_parse_planning_recovery_arg(
    arg: &str,
    iter: &mut ArgIter,
    state: &mut CliParseState,
) -> Result<bool, ConfigError> {
    try_parse_planning_recovery_option(
        arg,
        iter,

...
mod tests {
    ...
    #[test]
    fn functional_signer_preflight_quorum_decision_path_matrix() {
        // Regression: #3957
        let _lock = test_signer_env_lock()
            .lock()
            .expect("signer env lock should guard test mutation");
        ...
        let secondary_selection = KolmeLiveSignerSelection {
            profile: "ops-secondary",
            key_source: "env-local",
        ...
        let matrix = [
            Case {
                ...
                selection: test_primary_selection(),
        ...
        for case in matrix {
            ...
            let _rotation_epoch = EnvVarGuard::set(
                "KAMN_KOLME_LIVE_SIGNER_ROTATION_EPOCH",
                Some(case.rotation_epoch),
            );
            let _previous_rotation_epoch = EnvVarGuard::set(
            ...
            match case.expected_reason {
                Some(reason_code) => {
                    let error = evaluate_kolme_live_signer_preflight_readiness(&case.selection)
                ...
                None => {
                    let readiness = evaluate_kolme_live_signer_preflight_readiness(&case.selection)
                        .expect("matrix success case must stay ready");
                    assert!(

...
macro_rules! base_bootstrap_report {
    () => {
        NodeBootstrapReport {
...
fn assert_text_rendered_markers(rendered: &str) {
    assert!(rendered.contains("role: processor"));
    assert!(rendered.contains("planning_expected_state_hash: state-1"));
    assert!(rendered.contains("planning_scheduled_candidate_ids: tx-1, tx-2"));
    assert!(rendered.contains("recovery_expected_state_version: none"));
    assert!(rendered.contains("components: processor, listener"));
}

#[test]
fn functional_json_render_is_deterministic() {
    let report = base_bootstrap_report!();
...
#[test]
fn functional_text_render_preserves_none_and_joined_markers() {
    let report = NodeBootstrapReport {
        planning_expected_state_hash: Some("state-1".to_owned()),
        planning_candidate_count: Some(2),
        planning_scheduled_candidate_ids: Some(vec!["tx-1".to_owned(), "tx-2".to_owned()]),
        ..base_bootstrap_report!()
    };

    let rendered = render_bootstrap_report(&report, OutputMode::text());
    assert_text_rendered_markers(&rendered);
}

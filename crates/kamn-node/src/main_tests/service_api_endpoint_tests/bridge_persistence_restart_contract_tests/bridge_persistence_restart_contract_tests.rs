...
fn assert_bridge_restart_phase(caller_did: &str, bridge_id: String, state_file: &std::path::Path) {
    ...
    assert_bridge_restart_payloads(
        &forwarded,
        &queried,
        &missing,
        &state_json,
...
fn assert_bridge_restart_payloads(
    ...
    assert_eq!(
        state_json["bridges"][bridge_id]["bridge_status"],
        "forwarded"
    );
    assert_eq!(
        state_json["bridges"][bridge_id]["target_message_id"],
        format!("msg-bridge-target-{bridge_id}")
    );
    assert_eq!(
        state_json["bridges"][bridge_id]["forward_tx_hash"],

...
fn integration_service_api_endpoint_persists_bridge_state_across_restart() {
    ...
    let forwarded = forward_bridge(&restart_snapshot, restart_bind_addr.as_str(), caller_did, 102, bridge_id);
    let queried = query_bridge(&restart_snapshot, restart_bind_addr.as_str(), caller_did, 103, bridge_id);

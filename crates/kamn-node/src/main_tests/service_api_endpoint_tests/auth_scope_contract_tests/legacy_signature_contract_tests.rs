...
fn integration_service_api_endpoint_rejects_legacy_deterministic_signature_profile() {
    ...
    let response = legacy_signature_rejection_response(
        &snapshot,
        bind_addr.as_str(),
        "kamn:did:agent:test-client-legacy-signature",
        r#"{"message":"legacy-signature"}"#,
    );
...
fn regression_service_api_endpoint_rejects_legacy_signature_when_toggle_env_is_true() {
    ...
    let response = legacy_signature_rejection_response(
        &snapshot,
        bind_addr.as_str(),
        "kamn:did:agent:test-client-legacy-toggle-true",
        r#"{"message":"legacy-signature-toggle-true"}"#,
    );
    assert!(response.contains("HTTP/1.1 401 Unauthorized"));
    let error_payload = parse_error_envelope_from_http_response(response.as_str());
    assert_eq!(
...
fn legacy_signature_rejection_response(

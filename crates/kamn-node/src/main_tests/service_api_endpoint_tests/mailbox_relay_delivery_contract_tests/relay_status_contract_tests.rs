use super::super::*;
use super::support::{
    assert_server_ok, build_mailbox_relay_snapshot, query_message, read_state_json,
...
fn regression_service_api_endpoint_recipient_query_requires_relayed_state_before_delivery() {
    ...
    let payload = send_message(&snapshot, bind_addr.as_str(), "kamn:did:agent:delivery-gate-sender", 71, r#"{"recipient_did":"kamn:did:agent:delivery-gate-recipient","message":"deliver-gate"}"#);
    let query = query_message(&snapshot, bind_addr.as_str(), "kamn:did:agent:delivery-gate-recipient", 72, payload.message_id.as_str());
    assert_eq!(query["message_id"], payload.message_id);
    assert_eq!(query["status"], "created");
    assert_server_ok(server, "service api endpoint should stop cleanly after delivery gate regression flow");
    assert_eq!(read_state_json(state_file.as_path())["messages"][payload.message_id.as_str()]["status"], "created");
...
fn integration_service_api_endpoint_recipient_query_promotes_relayed_to_delivered() {
    ...
    assert_eq!(query_message(&snapshot, bind_addr.as_str(), recipient_did.as_str(), 51, "msg-relayed-to-delivered-1")["status"], "delivered");
    assert_server_ok(server, "service api endpoint should stop cleanly after relayed recipient query flow");
    let _ = fs::remove_file(state_file);
}
...
fn assert_non_recipient_restart_phase(api_bind: &str, nonce: u64, state_file: &Path) {
    ...
    assert_server_ok(server, "service api endpoint should stop cleanly after non-recipient relay query");
    assert_eq!(read_state_json(state_file)["messages"]["msg-relayed-non-recipient-restart-1"]["status"], "relayed");
}

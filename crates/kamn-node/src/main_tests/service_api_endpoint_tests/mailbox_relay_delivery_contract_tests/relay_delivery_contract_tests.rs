...
fn integration_service_api_endpoint_enqueues_recipient_relays_to_durable_spool() {
    ...
    super::support::assert_server_ok(server, "service api endpoint should stop cleanly after relay spool enqueue request");
    let spool_entry = read_first_spool_entry(spool_file.as_path());
    assert_eq!(spool_entry["message_id"], payload.message_id);
    assert_eq!(spool_entry["sender_did"], test_service_api_sender_did(sender_did));
...
impl CrossNodeFiles {
    fn new() -> Self {
        Self {
            sender_state_file: unique_named_state_file("kamn-node-service-api-cross-node-sender-state"),
            sender_spool_file: unique_named_relay_spool_file("kamn-node-service-api-cross-node-sender-spool"),
            recipient_state_file: unique_named_state_file("kamn-node-service-api-cross-node-recipient-state"),
            recipient_spool_file: unique_named_relay_spool_file("kamn-node-service-api-cross-node-recipient-spool"),
        }
    }
}
...
fn boot_sender(files: &CrossNodeFiles) -> ServerContext {
    ...
    let bind_addr = reserve_loopback_addr();
    let server = spawn_api_server(&snapshot, bind_addr.as_str(), 1);
    ServerContext { bind_addr, snapshot, server, _state_guard: state_guard, _spool_guard: spool_guard }
}
...
fn assert_cross_node_send_phase(sender: ServerContext, sender_did: &str, recipient_did: &str) -> ServiceApiMessageCreateBody {
    let body = format!(r#"{{"recipient_did":"{recipient_did}","message":"cross-node"}}"#);
    let payload = send_message(&sender.snapshot, sender.bind_addr.as_str(), sender_did, 81, body.as_str());
    super::support::assert_server_ok(sender.server, "sender endpoint should stop cleanly after send request");
    payload
}
...
fn project_relay_to_recipient(files: &CrossNodeFiles, recipient_addr: &str, recipient_did: &str) {
    ...
    let _route_guard = EnvVarGuard::set("KAMN_SERVICE_API_RELAY_RECIPIENT_ROUTE_MAP_JSON", Some(route_map.as_str()));
    let _key_guard = EnvVarGuard::set("KAMN_SERVICE_API_AUTH_PRIVATE_KEY_HEX", Some(TEST_SERVICE_API_AUTH_PRIVATE_KEY_HEX));
    let report = run_daemon_once();
    assert_eq!(report.runtime_mode, "daemon");
}

fn assert_cross_node_recipient_phase(recipient: ServerContext, recipient_did: &str, sender_did: &str, payload: &ServiceApiMessageCreateBody) {
    let mailbox = super::support::list_mailbox(&recipient.snapshot, recipient.bind_addr.as_str(), recipient_did, 82, recipient_did);
    let message = query_message(&recipient.snapshot, recipient.bind_addr.as_str(), recipient_did, 83, payload.message_id.as_str());
    assert_eq!(mailbox.messages.iter().filter(|id| *id == &payload.message_id).count(), 1);
    assert_eq!(message["message_id"], payload.message_id);
    assert_eq!(message["status"], "delivered");
    assert_eq!(message["sender_did"], test_service_api_sender_did(sender_did));
    assert_eq!(message["recipient_did"], recipient_did);
    super::support::assert_server_ok(recipient.server, "recipient endpoint should stop cleanly after relay delivery checks");
}
...
fn assert_cross_node_persistence(files: &CrossNodeFiles, message_id: &str) {
    ...
    assert!(fs::read_to_string(files.sender_spool_file.as_path()).expect("sender relay spool file should remain readable").trim().is_empty());
}

...
fn assert_route_lifecycle_queries(snapshot: &ServiceApiSnapshot) {
    let expired = render_service_api_endpoint_response(
        snapshot,
        "POST",
    ...
    let tombstoned = render_service_api_endpoint_response(
        snapshot,
        "POST",
    ...
    let bridge_forward_response =
        render_service_api_endpoint_response(snapshot, "POST", "/v1/bridge/bridge-1/forward", "{}");

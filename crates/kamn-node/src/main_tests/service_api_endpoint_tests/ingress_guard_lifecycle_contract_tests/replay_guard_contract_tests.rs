...
fn regression_service_api_endpoint_rejects_replayed_request_nonce_for_sender() {
    ...
    let first_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 7, body);
    let replay_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 7, body);
...
fn integration_service_api_endpoint_replay_rejection_remains_stable_with_anti_spam_enforcement() {
    ...
    let server = spawn_ingress_server(&snapshot, 3, 2_000, DEFAULT_SERVICE_API_BODY_LIMIT_BYTES, DEFAULT_SERVICE_API_CONCURRENCY_LIMIT, 1_000);
    let body = "{\"message\":\"replay-anti-spam-matrix\"}";
    let sender_did = "kamn:did:agent:test-client-replay-anti-spam";
    let first_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 701, body);
    let replay_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 701, body);
    let fresh_nonce_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 702, body);
...
fn assert_replay_rejection(response: &str) {
    ...
    assert_eq!(payload.reason_code, "service_api_auth_replay_nonce_detected");
...
fn replay_duplicate_sequence(
    ...
    for round in 0..rounds {
        ...
        let first = send_signed_message_request(snapshot, bind_addr, sender_did, nonce, body.as_str());
        assert!(first.contains("HTTP/1.1 202 Accepted"));
        observed.push("accepted".to_owned());
        let replay = send_signed_message_request(snapshot, bind_addr, sender_did, nonce, body.as_str());

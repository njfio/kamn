...
fn functional_service_api_endpoint_rejects_when_rate_limit_is_exceeded() {
    ...
    let server = spawn_ingress_server(&snapshot, 2, 2_000, DEFAULT_SERVICE_API_BODY_LIMIT_BYTES, DEFAULT_SERVICE_API_CONCURRENCY_LIMIT, 1);
    let sender_did = "kamn:did:agent:test-client-rate-limit";
    let message_body = "{\"message\":\"rate-limit-check\"}";

    let first_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 101, message_body);
    let second_response = send_signed_message_request(&snapshot, server.bind_addr.as_str(), sender_did, 102, message_body);
    ...
    assert_eq!(payload.reason_code, "service_api_ingress_rate_limit_exceeded");
...
fn regression_service_api_endpoint_oversized_payload_maps_body_limit_reason_code() {
    ...
    let server = spawn_ingress_server(&snapshot, 1, 2_000, 256, DEFAULT_SERVICE_API_CONCURRENCY_LIMIT, 1_000);
    ...
    assert_eq!(payload.reason_code, "service_api_ingress_body_size_limit_exceeded");
...
fn regression_service_api_endpoint_unauthorized_ingress_consumes_request_budget() {
    ...
    let response = send_http_request(server.bind_addr.as_str(), "POST", "/v1/messages/send", "{\"message\":\"unsigned\"}");
    ...
    assert_server_ok(server.server, "unauthorized ingress must still consume request budget for graceful shutdown");
}

...
pub(super) fn build_task_escrow_snapshot(api_bind: &str) -> ServiceApiSnapshot {
...
pub(super) fn fund_escrow(
    ...
    let response = signed_request(snapshot, bind_addr, 1, "POST", "/v1/escrow/fund", caller_did, nonce, payload);
    assert!(response.contains("HTTP/1.1 200 OK"));
    parse_service_api_payload(extract_http_response_body(response.as_str()))

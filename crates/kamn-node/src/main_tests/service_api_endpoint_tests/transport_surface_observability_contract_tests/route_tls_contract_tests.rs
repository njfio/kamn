...
fn integration_service_api_endpoint_serves_required_http_routes() {
    ...
    let send_response = send_signed_message_request(
        &snapshot,
        server.bind_addr.as_str(),
        "kamn:did:agent:test-client-1",
        1,
        "{\"message\":\"hello\"}",
    );
...
fn integration_service_api_endpoint_async_runtime_handles_concurrent_http_routes() {
    ...
    let ended_cleanly_or_timeout = result.is_ok()
        || result
            .as_ref()
...
fn integration_service_api_endpoint_tls_mode_serves_required_https_routes() {
    ...
    let health = send_https_request_with_headers(
        server.bind_addr.as_str(),
        "GET",
        "/healthz",
        "",
    ...
    let metrics = send_https_request_with_headers(
        server.bind_addr.as_str(),
        "GET",
        "/metrics",
        "",
    ...
    assert!(
        metrics.contains("kamn_service_api_observability_source{source=\"service-api-runtime\"} 1")
    );
    assert_server_ok(
        server.server,
...
fn integration_service_api_endpoint_http_response_bodies_match_serde_contracts() {
    ...
    let server = spawn_transport_server(&snapshot, 2);
    let send = send_signed_message_request(
        &snapshot,
        server.bind_addr.as_str(),
        "kamn:did:agent:test-client-serde",
        31,
        "{\"message\":\"serde-live\"}",
    );
    ...
    let send_payload: ServiceApiMessageCreateBody =
        parse_service_api_payload(extract_http_response_body(send.as_str()))
            .expect("send payload should deserialize");
    let health_payload: ServiceApiHealthBody =
        parse_service_api_payload(extract_http_response_body(health.as_str()))

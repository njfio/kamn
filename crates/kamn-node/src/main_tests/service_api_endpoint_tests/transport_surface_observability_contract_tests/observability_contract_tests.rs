use super::super::*;
use super::observability_support::{assert_correlation_markers, spawn_correlation_client};
...
fn regression_service_api_runtime_observability_projects_live_metrics_under_traffic() {
    ...
    let signature = service_api_request_signature_for_fields(
        "kamn:did:agent:runtime-observability",
        1,
        super::support::state_hash(&snapshot).as_str(),
        body,
    );
    let send = send_http_request_with_headers(
        server.bind_addr.as_str(),
        "POST",
        "/v1/messages/send",
        body,
        &[
            ("X-KAMN-Sender-DID", "kamn:did:agent:runtime-observability"),
            ("X-KAMN-Request-Nonce", "1"),
            ("X-KAMN-Request-Signature", signature.as_str()),
            ("X-KAMN-Authz-Scope", "messages:write"),
        ],
    );
    let unauth = send_http_request(
        server.bind_addr.as_str(),
        "POST",
        "/v1/messages/send",
        "{\"message\":\"unauth\"}",
...
fn functional_service_api_endpoint_emits_structured_ingress_correlation_markers() {
    ...
    let client = spawn_correlation_client(
        bind_addr.clone(),
        super::support::state_hash(&snapshot).as_str(),
    );

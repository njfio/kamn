...
fn integration_service_api_endpoint_registers_agent_metadata_idempotently_and_conflicts_on_mismatch() {
    ...
    let registration_payload = register_agent(&snapshot, bind_addr.as_str(), caller_did, 201, registration_body);
    let duplicate_response = raw_signed_request(&snapshot, bind_addr.as_str(), 1, "POST", "/v1/agents/register", caller_did, 202, registration_body, &[]);
    ...
    assert_eq!(registration_payload.capabilities, vec!["text".to_owned(), "code".to_owned()]);
    assert!(duplicate_response.contains("HTTP/1.1 201 Created"));
    assert!(mismatch_response.contains("HTTP/1.1 409 Conflict"));
    assert_eq!(parse_error_envelope(extract_http_response_body(mismatch_response.as_str())).reason_code, "service_api_agent_registration_conflict");
    ...
    assert_eq!(query_payload.capabilities, vec!["text".to_owned(), "code".to_owned()]);
}
...
fn integration_service_api_endpoint_searches_registered_agent_metadata() {
    ...
    for (nonce, caller_did, registration_body) in [
        (401_u64, "kamn:did:agent:search-alpha", r#"{"agent_type":"assistant","model_family":"gpt-5","capabilities":["text","code"]}"#),
        (402_u64, "kamn:did:agent:search-beta", r#"{"agent_type":"assistant","model_family":"gpt-4.1","capabilities":["text"]}"#),
    ] {
        let response = raw_signed_request(&snapshot, bind_addr.as_str(), 1, "POST", "/v1/agents/register", caller_did, nonce, registration_body, &[]);
    ...
    assert_eq!(search_payload[0].did, test_service_api_sender_did("kamn:did:agent:search-alpha"));
    assert_eq!(search_payload[0].agent_type, "assistant");
    assert_eq!(search_payload[0].model_family, "gpt-5");
    assert_eq!(search_payload[0].capabilities, vec!["text".to_owned(), "code".to_owned()]);
...
fn integration_service_api_endpoint_rejects_invalid_agent_search_payload() {
    ...
    let payload: ServiceApiErrorBody = parse_service_api_payload(extract_http_response_body(response.as_str()))
        .expect("error payload should deserialize");
    assert_eq!(payload.error, "bad-request");
    assert_eq!(payload.reason_code, "service_api_agent_search_payload_invalid");
    assert!(payload.message.contains("agent search payload capability must not be empty when provided"));
}

fn role_profile_ids_csv(profiles: &[LivePostgresLoadProfile]) -> String {
    profiles
        .iter()
...
fn role_profile_reason_codes(profiles: &[LivePostgresLoadProfile]) -> Vec<&'static str> {
    profiles
        .iter()
...
fn functional_runtime_daemon_live_postgres_validation_slice_role_profile_matrix_contract_is_canonical(
    ...
    assert_eq!(role_profile_ids_csv(&profiles), LIVE_POSTGRES_MATRIX_ROLE_PROFILE_IDS_CSV);
    assert_eq!(
        role_profile_reason_codes(&profiles),
        vec![

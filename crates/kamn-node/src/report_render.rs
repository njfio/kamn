...
fn render_text_report(report: &NodeBootstrapReport) -> String {
    ...
    let daemon_live_postgres_multi_host_execution_bundle_row_count = report
        ...
        .map(|value| value.to_string())
        .unwrap_or_else(|| "none".to_owned());
    let daemon_live_postgres_multi_host_execution_bundle_selector_rows_fingerprint = report
...
fn render_json_report(report: &NodeBootstrapReport) -> String {
    ...
    let daemon_live_postgres_multi_host_execution_bundle_selector_rows_fingerprint =
        match &report.daemon_live_postgres_multi_host_execution_bundle_selector_rows_fingerprint {
            Some(value) => format!("\"{}\"", json_escape(value)),
            None => "null".to_owned(),
        };

...
pub(super) fn render_service_api_endpoint_response(
    ...
    if method == "POST" {
        if let Some(task_id) = task_accept_path_id(path) {
            let payload = ServiceApiTaskAcceptBody {
        ...
        if let Some(task_id) = task_complete_path_id(path) {
            let payload = ServiceApiTaskCompleteBody {
                task_id: task_id.to_owned(),
        ...
        if path == ROUTE_ESCROW_FUND {
            ...
            let payload = ServiceApiEscrowFundBody {
                escrow_id,
                task_id: body_task_id(body).unwrap_or_else(|| "task-unknown".to_owned()),
                status: "funded".to_owned(),
            };
            return ServiceApiEndpointResponse {
                status_code: 201,
                content_type: "application/json",
        ...
        if let Some(escrow_id) = escrow_release_path_id(path) {
            let payload = ServiceApiEscrowReleaseBody {
...
fn body_task_id(body: &str) -> Option<String> {

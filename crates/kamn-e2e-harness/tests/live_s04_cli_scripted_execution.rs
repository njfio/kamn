...
fn integration_live_s04_cli_scripted_task_lifecycle_probe_against_local_runtime() {
...
fn require_env(key: &str) {
    let value = std::env::var(key)
        .unwrap_or_else(|_| panic!("required env missing for live CLI S-04 probe: {key}"));
    assert!(
        !value.trim().is_empty(),
        "required env must not be empty for live CLI S-04 probe: {key}"
    );
}

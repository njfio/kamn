...
fn spec_c07_verify_command_writes_deterministic_report_output() {
    ...
    prepare_verify_fixture(&evidence_dir, &chain_dump_path);
    let config = VerifyCommandConfig {
        evidence_dir: evidence_dir.display().to_string(),
        kolme_chain_dump: chain_dump_path.display().to_string(),
        output: output_path.display().to_string(),
    };
    assert_verify_report_is_deterministic(&config, &output_path);
    cleanup_verify_fixture(&evidence_dir, &output_path, &chain_dump_path);
}

fn prepare_verify_fixture(evidence_dir: &Path, chain_dump_path: &Path) {
    std::fs::create_dir_all(evidence_dir).expect("evidence dir should be created");
    std::fs::write(evidence_dir.join("manifest.json"), verify_manifest_json())
        .expect("manifest should be written");
    std::fs::write(chain_dump_path, valid_chain_dump_json()).expect("chain dump should be written");
}
...
fn cleanup_verify_fixture(evidence_dir: &Path, output_path: &Path, chain_dump_path: &Path) {
...
fn verify_manifest_json() -> &'static str {

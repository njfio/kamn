...
fn spec_c01_command_rejects_missing_three_agent_view_artifacts() {
    let root = temp_root("missing-views");
    mvp_local_artifacts::write_valid_local_artifacts(&root);
    write_transcript(&root, transcript(None));
    let report = write_report(&root, report_json(&root, None));
...
fn spec_c02_command_rejects_agent_c_private_overdisclosure() {
    let root = temp_root("agent-c-private");
    mvp_local_artifacts::write_valid_local_artifacts(&root);
    write_view_artifacts(&root, Some(agent_c_private_view()));
    write_transcript(&root, transcript(Some(&root)));
    let report = write_report(&root, report_json(&root, Some(&root)));
...
fn spec_c03_command_rejects_mismatched_view_settlement_signature() {
    let root = temp_root("mismatched-view-signature");
    mvp_local_artifacts::write_valid_local_artifacts(&root);
    write_view_artifacts(
        &root,
        Some(agent_c_public_view().replace(SIGNATURE, "mismatch")),
    );
    write_transcript(&root, transcript(Some(&root)));
    let report = write_report(&root, report_json(&root, Some(&root)));
...
fn write_transcript(root: &Path, transcript: String) {
...
fn write_view_artifacts(root: &Path, agent_c: Option<String>) {
    mvp_local_artifacts::write_file(
...
fn report_json(root: &Path, views_root: Option<&Path>) -> String {
    format!(
...
fn three_agent_claim(root: &Path, views_root: Option<&Path>) -> String {
    let view_fields = views_root.map(view_claim_fields).unwrap_or_default();
    format!(
...
fn transcript(views_root: Option<&Path>) -> String {
    let view_bindings = views_root.map(transcript_view_fields).unwrap_or_default();
    format!(
...
fn agent_c_private_view() -> String {

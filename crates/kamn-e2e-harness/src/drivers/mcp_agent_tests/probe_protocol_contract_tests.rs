...
fn spec_c02_parse_framed_jsonrpc_payloads_supports_multiple_frames() {
    ...
    assert_eq!(
        payloads,
        vec![INIT_PAYLOAD.to_owned(), HEALTH_PAYLOAD.to_owned()]
    );
}
...
fn spec_c04_parse_framed_jsonrpc_payloads_accepts_leading_newlines() {
    ...
    let payloads =
        parse_framed_jsonrpc_payloads(framed.as_str()).expect("leading newline should be skipped");
...
fn spec_c23_validate_probe_initialize_response_accepts_required_markers() {
    let payload =
        r#"{"jsonrpc":"2.0","id":"probe-init","result":{"serverInfo":{"name":"kamn-mcp-server"}}}"#;

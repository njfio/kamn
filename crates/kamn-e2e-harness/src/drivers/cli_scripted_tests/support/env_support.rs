...
{
    let mut updates = vec![(CLI_BINARY_ENV, Some("/definitely/missing/kamn-cli"))];
    updates.extend_from_slice(extra_updates);
    with_env_vars(&updates, || assert_spawn_failure(runner()));
}

pub(crate) fn assert_scripted_probe_succeeds<F>(
    stem: &str,
    source: &str,
    extra_updates: &[(&str, Option<&str>)],
    runner: F,
    ...
    F: FnOnce() -> Result<(), String>,
{
    with_scripted_probe_env(stem, source, extra_updates, || {
        runner().expect("scripted probe should succeed");
    });
}

pub(crate) fn assert_scripted_probe_error_contains<F>(
    stem: &str,
    source: &str,
    extra_updates: &[(&str, Option<&str>)],
    expected: &str,
    ...
    F: FnOnce(),
{
    ...
    let mut updates = vec![(CLI_BINARY_ENV, Some(script_binary.as_str()))];
    updates.extend_from_slice(extra_updates);

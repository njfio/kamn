...
pub(crate) fn run_service_contract_server_with_websocket_payload(
    ...
    run_bound_service_contract_server_with_websocket_payload(
        listener,
        max_requests,
        websocket_payload,
    )
...
pub(crate) fn run_bound_service_contract_server(
...
fn run_bound_service_contract_server_with_websocket_payload(
    listener: TcpListener,
    max_requests: u64,
    websocket_payload: String,
) -> Result<(), String> {
...
fn write_public_or_auth_failure(
    ...
    if public_route_support::write_public_response(
        stream,
        method,
        path,
        headers,

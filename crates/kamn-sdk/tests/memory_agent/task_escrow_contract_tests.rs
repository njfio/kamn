...
fn get_task_status_reports_submitted_accepted_and_completed_states() {
    ...
    assert_eq!(client.get_task_status(&task_id).expect("accepted status").state, "accepted");
    client.complete_task(&task_id).expect("complete task should succeed");
    assert_eq!(client.get_task_status(&task_id).expect("completed status").state, "completed");
}
...
fn escrow_moves_balances_from_payer_to_payee() {
    ...
    client.release_escrow(&escrow_id).expect("release escrow failed");
...
fn submit_artifact_and_complete_task_flow() {
    ...
    client.complete_task(&task_id).expect("complete task failed");
    assert_eq!(

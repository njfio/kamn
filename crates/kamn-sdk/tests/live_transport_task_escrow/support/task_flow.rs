use crate::support::{did, deterministic_u64_tag, live_artifact, live_escrow, live_task};
use kamn_sdk::{
...
pub(crate) fn assert_task_flow(client: &mut LiveTransportKamnClient) {
    ...
    client.complete_task(&task_id).expect("complete_task should succeed");
...
pub(crate) fn assert_escrow_flow(client: &mut LiveTransportKamnClient) -> EscrowId {
    let escrow_id = client.create_escrow(live_escrow()).expect("create_escrow should succeed");
...
pub(crate) fn assert_balance_route_fails_closed(client: &LiveTransportKamnClient) {
    assert_eq!(
        ...
        Err(SdkError::TransportFailure("failed to connect to service endpoint"))
    );
}
...
fn submit_and_verify_artifact(client: &mut LiveTransportKamnClient, task_id: &TaskId) -> ArtifactId {
    let artifact_id = client.submit_artifact(task_id, live_artifact()).expect("submit_artifact should succeed");
    assert_eq!(artifact_id, ArtifactId(deterministic_u64_tag("content-local-artifact-abc")));
    let artifact_status = client.get_artifact_status(&artifact_id).expect("get_artifact_status should succeed");
...
fn verify_expire_roundtrip(client: &mut LiveTransportKamnClient, artifact_id: &ArtifactId) {
    let expired = client.expire_artifact(artifact_id).expect("expire_artifact should succeed");
    assert_artifact_status(&expired, artifact_id, "expired", "none");
    let reread = client.get_artifact_status(&expired.artifact_id).expect("get_artifact_status after expire should succeed");
    assert_eq!(reread, expired);
}

fn verify_tombstone_roundtrip(client: &mut LiveTransportKamnClient, artifact_id: &ArtifactId) {
    let tombstoned = client.tombstone_artifact(artifact_id).expect("tombstone_artifact should succeed");
    assert_artifact_status(&tombstoned, artifact_id, "tombstoned", "redacted");
    let reread = client.get_artifact_status(&tombstoned.artifact_id).expect("get_artifact_status after tombstone should succeed");
    assert_eq!(reread, tombstoned);
}

fn assert_task_status_state(client: &LiveTransportKamnClient, task_id: &TaskId, expected_state: &str) {
    assert_eq!(client.get_task_status(task_id).expect("task status should succeed").state, expected_state);
}

fn assert_artifact_status(status: &ArtifactStatus, artifact_id: &ArtifactId, lifecycle_state: &str, redaction_status: &str) {
    assert_eq!(
        status,
        &ArtifactStatus {

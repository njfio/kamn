use crate::support::{
    assert_escrow_flow, assert_task_flow, ensure_live_test_env, live_client, spawn_expected_server,
    task_and_escrow_requests,
};
...
fn spec_c06_live_transport_task_and_escrow_routes_execute_network_contract() {
    ...
    let (bind_addr, server) = spawn_expected_server(task_and_escrow_requests());

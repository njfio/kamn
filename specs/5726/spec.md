# Spec: #5726 Execute R52 Spec-Volume Remediation Tranche-5 (14-Dir Reduction)
...
## Problem Statement
R52 tranche-4 lowered top-level `specs/` directories to 797, but spec-volume pressure remains above
remediation target. A fifth deterministic tranche is required to continue downward movement while
preserving archive-policy and fail-closed docs-contract integrity.
...
- Refresh R50 non-regression ratchet markers in `docs/review/gaps-and-issues-r50.md`.
- Extend existing docs-contract test expectations for tranche-5 values.
...
## Acceptance Criteria
### AC-1 Deterministic tranche-5 reduction
Given tranche-5 pre-count evidence for top-level `specs/` directories,
When tranche-5 cleanup executes,
...
Then expected tranche-5 values exist and satisfy `pre - deleted = post`.
...
- Top-level `specs/` directory count decreases by 14 in tranche-5.

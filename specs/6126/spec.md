# Spec: Issue #6126 - Task: [S-03] Replace custom pipe-delimited snapshot format with serde
...
## Problem Statement
Suggestion from R59 section 8. Impact: ~200 lines of manual serialize/parse per domain type

## Scope
In scope:
- Implement targeted remediation for `S-03` from `docs/review/gaps-and-issues-r59-swarm.md`.
...
Out of scope:
- Unrelated refactors outside `S-03`.
...
## Acceptance Criteria
- AC-1: The S-03 gap is remediated with production-safe behavior.
...
## Conformance Cases
- C-01 (Conformance, AC-1): Implemented behavior resolves R59 S-03 with deterministic pass/fail signals.
...
- Targeted R59 finding `S-03` no longer appears as unresolved in follow-up review docs.

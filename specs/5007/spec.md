...
## Problem Statement
Implement PRD M4 escrow integration: escrow state machine persistence, escrow-scoped message flows, auditor key handling, and settlement evidence linkage.

## Acceptance Criteria
- AC-1: Scope for issue #5007 is decomposed into explicit implementation/integration/validation outcomes with deterministic test evidence.
...
## Scope
In scope:
- Issue-specific delivery for Story: M4 escrow-scoped messaging and settlement evidence integration.
...
|---|---|---|---|---|
| C-01 | AC-1 | Functional | Execute issue task plan for #5007 | Planned implementation/integration steps are completed with evidence |
| C-02 | AC-2 | Conformance | Run mapped test commands for #5007 | All mapped conformance checks pass and produce deterministic markers |
...
## Success Metrics
- Issue #5007 reaches `Status: Implemented` with ACs mapped to passing conformance evidence.

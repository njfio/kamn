...
- Title: Task: build superseded-script inventory and deterministic deletion-manifest contracts
- Status: Implemented
- Type: task
- Priority: P0
...
## Problem Statement
Issue #4958 is part of the R27.44 shell maintainability tranche and closes one portion of the deletion-wave, spec-archival, and hard-ceiling governance gap.

## Acceptance Criteria
- AC-1: Scope defined in GitHub issue #4958 is implemented and verified.
...
## Scope
In scope:
- Work explicitly described in issue #4958.
...
|---|---|---|---|---|
| C-01 | AC-1 | Functional | Execute scoped workflow for #4958 | Behavior matches issue acceptance criteria |
...
- AC-2:
  - `bash scripts/ci/test_check_superseded_script_deletion_manifest.sh`
    (schema drift, unknown-script reference, and missing replacement evidence fail-closed cases)
- AC-3:
  - `bash scripts/ci/test_check_superseded_script_deletion_manifest.sh`
...
## Success Metrics
- All ACs for #4958 are mapped to conformance cases and passing tests.
- No shell-surface governance regressions introduced by #4958.

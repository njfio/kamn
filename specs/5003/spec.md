...
## Problem Statement
Implement PRD M0 foundation: core PostgreSQL schema, append-only controls, envelope encrypt/decrypt, zstd compression, and hash-chain verification.

## Acceptance Criteria
- AC-1: Scope for issue #5003 is decomposed into explicit implementation/integration/validation outcomes with deterministic test evidence.
...
## Scope
In scope:
- Issue-specific delivery for Story: M0 foundation schema, append-only enforcement, and envelope crypto pipeline.
...
|---|---|---|---|---|
| C-01 | AC-1 | Functional | Execute issue task plan for #5003 | Planned implementation/integration steps are completed with evidence |
| C-02 | AC-2 | Conformance | Run mapped test commands for #5003 | All mapped conformance checks pass and produce deterministic markers |
...
## Success Metrics
- Issue #5003 reaches `Status: Implemented` with ACs mapped to passing conformance evidence.

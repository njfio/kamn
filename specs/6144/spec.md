...
## Problem Statement
Cut governance-only contract/test overhead while preserving essential policy guarantees.

## Scope
In scope:
- Implement targeted remediation for `X-06` from `docs/review/gaps-and-issues-r59-swarm.md`.
...
Out of scope:
- Unrelated refactors outside `X-06`.
...
## Acceptance Criteria
- AC-1: The X-06 gap is remediated with production-safe behavior.
...
## Conformance Cases
- C-01 (Conformance, AC-1): Implemented behavior resolves R59 X-06 with deterministic pass/fail signals.
...
- Targeted R59 finding `X-06` no longer appears as unresolved in follow-up review docs.

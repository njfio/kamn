...
## Acceptance criteria
- [ ] `crates/kamn-node/tests/main_module_extraction_contract.rs` is reduced to a thin root shell at or below 180 LOC
- [ ] Root shell wires bounded sibling modules for the current main-module extraction concerns and shared support
- [ ] All extracted files touched by the split remain at or below 200 LOC
- [ ] `cargo test -p kamn-node --test main_module_extraction_contract_extraction_contract -- --nocapture` passes
- [ ] `cargo test -p kamn-node --test main_module_extraction_contract -- --nocapture` passes
- [ ] `python3 scripts/ci/check_touched_rust_size_policy.py --repo-root /tmp/kamn-6784-remote --base-ref origin/main --output-json /tmp/main-module-extraction-contract-size.json` returns `policy_decision=GO`

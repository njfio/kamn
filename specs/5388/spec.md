...
- Title: Task: codify topology-id host-mode-host-pair-cardinality coherence contracts for live-postgres validation
- Status: Reviewed
- Type: task
- Priority: P1
...
## Problem Statement
Topology-id host-mode-host-pair coherence and host-pair-cardinality coherence are explicit separately, but full host-mode-host-pair-cardinality coherence remains implicit. Drift that preserves partial coherences while violating combined coherence could bypass current contracts.

## Acceptance Criteria
- AC-1: daemon validation tests assert canonical topology-id->host-mode->host-pair->host-cardinality coherence schema/version and rows.
- AC-2: integration tests assert topology-id host-mode-host-pair-cardinality coherence rows remain deterministic and invariant under repeated runs and topology permutations.
- AC-3: `docs/ops/configuration.md` includes explicit `#5388` coherence markers and validation commands.
...
## Scope
In scope:
- Test-only host-mode-host-pair-cardinality coherence contracts in `crates/kamn-node/src/main_tests/daemon_tests.rs`.
...
|---|---|---|---|---|
| C-01 | AC-1 | Functional | topology host-mode-host-pair-cardinality coherence helper contracts | coherence schema version and canonical rows remain explicit/stable |
| C-02 | AC-2 | Integration | repeated/permuted topology fingerprints | extracted topology_id->host_mode->host_pair->host_cardinality rows remain identical under permutations |
...
## Success Metrics
- Topology-id host-mode/host-pair/host-cardinality coherence drift becomes explicitly detectable.
- Docs and tests fail closed on coherence marker regressions.
- R45 frontier narrative reflects host-mode-host-pair-cardinality coherence hardening.

...
- [ ] AC-1: `service_api_endpoint_tests.rs` declares a new task-escrow-persistence submodule and no longer retains the moved task/escrow persistence test markers.
- [ ] AC-2: Extracted task/escrow persistence files stay at or below 200 lines each.
- [ ] AC-3: The staged root threshold ratchets down from `5718` to `5350` lines or lower.
- [ ] AC-4: `cargo test -p kamn-node service_api_endpoint_tests_split_contract -- --nocapture` passes.
- [ ] AC-5: At least one extracted task/escrow persistence test passes from the real `kamn-node` test module path.

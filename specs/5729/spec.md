# Spec: #5729 Execute R52 Spec-Volume Remediation Tranche-6 (14-Dir Reduction)
...
## Problem Statement
R52 tranche-5 lowered top-level `specs/` directories to 784, but spec-volume pressure remains above
remediation target. A sixth deterministic tranche is required to continue reduction while preserving
...
- Extend existing docs-contract test expectations for tranche-6 values.
...
## Acceptance Criteria
### AC-1 Deterministic tranche-6 reduction
Given tranche-6 pre-count evidence for top-level `specs/` directories,
When tranche-6 cleanup executes,
...
Then expected tranche-6 values exist and satisfy `pre - deleted = post`.
...
- Top-level `specs/` directory count decreases by 14 in tranche-6.

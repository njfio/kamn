# Spec: Issue #6124 - Task: [S-01] Move 93 `*_docs.rs` test files to CI lint step
...
## Problem Statement
Suggestion from R59 section 8. Impact: Reduce test LOC by ~17K, faster compile, cleaner test metrics

## Scope
In scope:
- Implement targeted remediation for `S-01` from `docs/review/gaps-and-issues-r59-swarm.md`.
...
Out of scope:
- Unrelated refactors outside `S-01`.
...
## Acceptance Criteria
- AC-1: The S-01 gap is remediated with production-safe behavior.
...
## Conformance Cases
- C-01 (Conformance, AC-1): Implemented behavior resolves R59 S-01 with deterministic pass/fail signals.
...
- Targeted R59 finding `S-01` no longer appears as unresolved in follow-up review docs.

...
- moratorium activation base SHA configuration
...
- non-retroactive activation-baseline wiring so pre-moratorium history is excluded
...
- activation-base configuration is missing or invalid
...
- AC-4: Fast Gate invokes the existing checker with the rolling-window moratorium configuration and anchors the evaluated history at a moratorium activation base SHA rather than retroactively counting older history.
...
- CI wiring must not silently fall back to PR-local-only evaluation, retroactive full-history evaluation, or a weaker threshold.
...
- C-04 Integration/Red: workflow wiring regression test proves Fast Gate sources the moratorium activation config and passes the rolling-window flags and moratorium thresholds.
- C-05 Integration/Green: targeted doc/contract test proves CI strategy docs describe the 50-commit capability moratorium.

## Deviations
- Integration smoke showed that a retroactive `HEAD`-only window would immediately fail on pre-moratorium governance-heavy history (`13` governance / `37` feature in the latest 50 non-merge commits at the time of testing). The final implementation therefore anchors evaluation at `.ci/governance-feature-commit-ratio-moratorium.env` so the policy starts counting after the activation SHA instead of rewriting old history.

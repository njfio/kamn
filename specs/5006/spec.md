...
## Problem Statement
Implement PRD M3 search/indexing: blind-index computation, metadata/full-text indexes, and scoped search APIs.

## Acceptance Criteria
- AC-1: Scope for issue #5006 is decomposed into explicit implementation/integration/validation outcomes with deterministic test evidence.
...
## Scope
In scope:
- Issue-specific delivery for Story: M3 metadata and blind-index search surfaces.
...
|---|---|---|---|---|
| C-01 | AC-1 | Functional | Execute issue task plan for #5006 | Planned implementation/integration steps are completed with evidence |
| C-02 | AC-2 | Conformance | Run mapped test commands for #5006 | All mapped conformance checks pass and produce deterministic markers |
...
## Success Metrics
- Issue #5006 reaches `Status: Implemented` with ACs mapped to passing conformance evidence.

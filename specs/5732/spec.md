# Spec: #5732 Execute R52 Spec-Volume Remediation Tranche-7 (14-Dir Reduction)
...
## Problem Statement
R52 tranche-6 lowered top-level `specs/` directories to 771, but spec-volume pressure remains above
remediation target. A seventh deterministic tranche is required to continue reduction while
...
- Extend existing docs-contract test expectations for tranche-7 values.
...
## Acceptance Criteria
### AC-1 Deterministic tranche-7 reduction
Given tranche-7 pre-count evidence for top-level `specs/` directories,
When tranche-7 cleanup executes,
...
Then expected tranche-7 values exist and satisfy `pre - deleted = post`.
...
- Top-level `specs/` directory count decreases by 14 in tranche-7.

# Spec: Issue #6139 - Task: [X-01] Resolve failing functional shell ratio non-regression gate
...
## Problem Statement
Fix and stabilize `functional_shell_test_surface_ratio_non_regression_gate` failure reported in R59 baseline.

## Scope
In scope:
- Implement targeted remediation for `X-01` from `docs/review/gaps-and-issues-r59-swarm.md`.
...
Out of scope:
- Unrelated refactors outside `X-01`.
...
## Acceptance Criteria
- AC-1: The X-01 gap is remediated with production-safe behavior.
...
## Conformance Cases
- C-01 (Conformance, AC-1): Implemented behavior resolves R59 X-01 with deterministic pass/fail signals.
...
- Targeted R59 finding `X-01` no longer appears as unresolved in follow-up review docs.

...
## Acceptance criteria
- [ ] `signature_matches_supported_profile_for_fields()` uses the internal constant-time helper instead of direct signature equality.
- [ ] `GroupChannelCryptoEngine::decrypt()` uses the same helper for expected-vs-provided signature comparison.
- [ ] instruction verification uses the same helper for record-vs-claim signature comparison.
- [ ] helper fails closed on length mismatch and preserves equality behavior for matching bytes.
- [ ] regression tests fail if the scoped verification paths revert to direct equality.

...
- add a new docs-contract test for `#5376` lane-count mapping markers.
- run that exact test before docs updates to capture RED evidence.

2. Add mapping contracts in daemon tests:
- add explicit topology-id->lane-count mapping constants/helper assertions.
- add functional + integration tests for mapping stability and permutation invariance.

3. Extend docs marker contracts:
- add a `#5376` subsection in `docs/ops/configuration.md` with mapping markers and validation commands.

4. Update review narrative:
- refine `docs/review/gaps-and-issues-r45.md` next-frontier wording to include lane-count mapping hardening.
...
- Risk: mapping assertions overlap with lane-set mapping coverage.
  - Mitigation: enforce explicit topology-id keyed lane-count rows and assert exact cardinality values.
...
- New contract: topology ids map to canonical lane counts and remain stable under permutations.

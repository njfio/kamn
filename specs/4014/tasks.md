...
## Ordered Tasks
- [x] T1 (RED): add Rust checker tests for pass path, tampered-report failure, selector/workflow exclusion drift, docs remediation parity drift, and runtime budget contract (`C-01`..`C-06`).
- [x] T2 (GREEN): add threshold fixture under `fixtures/ci/` and parser validation (`C-03`).
- [x] T3 (GREEN): implement durability CI dry-run checker in `scripts/ci/` (`C-01`, `C-02`, `C-04`, `C-05`, `C-06`).
- [x] T4 (GREEN): update strategy/ops docs with checker, threshold, reason taxonomy, and remediation markers (`C-05`).
- [x] T5 (Integration): wire checker contract test into `scripts/ci/test_ci_tools.sh` fast/full paths (`C-04`).
- [x] T6 (VERIFY): run targeted fmt/clippy/tests and shell-surface guardrails (`C-06`).

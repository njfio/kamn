...
- [ ] AC-1: `service_api_endpoint_tests.rs` declares a new message-persistence submodule and no longer retains the moved message-persistence test markers.
- [ ] AC-2: Extracted message-persistence files stay at or below 200 lines each.
- [ ] AC-3: The staged root threshold ratchets down from `6923` to `6650` lines or lower.
- [ ] AC-4: `cargo test -p kamn-node service_api_endpoint_tests_split_contract -- --nocapture` passes.
- [ ] AC-5: At least one extracted message-persistence test passes from the real `kamn-node` test module path.

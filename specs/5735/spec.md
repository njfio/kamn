# Spec: #5735 Execute R52 Spec-Volume Remediation Tranche-8 (14-Dir Reduction)
...
## Problem Statement
R52 tranche-7 lowered top-level `specs/` directories to 758, but spec-volume pressure remains above
remediation target. An eighth deterministic tranche is required to continue reduction while
...
- Extend existing docs-contract test expectations for tranche-8 values.
...
## Acceptance Criteria
### AC-1 Deterministic tranche-8 reduction
Given tranche-8 pre-count evidence for top-level `specs/` directories,
When tranche-8 cleanup executes,
...
Then expected tranche-8 values exist and satisfy `pre - deleted = post`.
...
- Top-level `specs/` directory count decreases by 14 in tranche-8.

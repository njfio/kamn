# Spec: Issue #6142 - Task: [X-04] Add production K8s Service/Ingress/probes baseline
...
## Problem Statement
Provide concrete K8s Service/Ingress manifests and readiness/liveness probes.

## Scope
In scope:
- Implement targeted remediation for `X-04` from `docs/review/gaps-and-issues-r59-swarm.md`.
...
Out of scope:
- Unrelated refactors outside `X-04`.
...
## Acceptance Criteria
- AC-1: The X-04 gap is remediated with production-safe behavior.
...
## Conformance Cases
- C-01 (Conformance, AC-1): Implemented behavior resolves R59 X-04 with deterministic pass/fail signals.
...
- Targeted R59 finding `X-04` no longer appears as unresolved in follow-up review docs.
